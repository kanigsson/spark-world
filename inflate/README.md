...
| Package           | Contents |
|-------------------|----------|
| `Inflate`         | `Byte_Array` and the word types (Ore's, renamed), `Ore.Bits` under a short name, the `Status_Type` all layers report through |
...
The most recent recorded `gnatprove --level=4` run reported **7,833 checks,
...
gnatprove -P inflate.gpr -j16 --no-subprojects  # reproduce the proof
...
The proof switches are in `inflate.gpr` and `--no-subprojects` leaves Ore
to its own proof run. Bound the parallelism rather than passing `-j0`: the
memory limit applies per prover process, so the default 2 GB times one job
per core is more than a 32-core machine has, and the hardest checks here —
the `Spec_Walk` loop invariants in `Inflate.Dynamic` — are exactly the ones
that reach it. `-j16` peaks around 11 GB of provers and needs no raised
timeout; starving those checks of their default 60 seconds is what makes
them look unprovable.
