...
package body Fuzzy_Select is
   ...
   procedure Free is
     new Ada.Unchecked_Deallocation (Fuzzy.Search_Result_Array, Result_Buffer);
   ...
   is
      ...
      begin
         Fuzzy.Match_Details
           (Query (1 .. Length), Data, Slice, Matched, Value, Positions,
            Marked);
      ...
      begin
         if Selected > 0 then
            if Selected < Top then
              ...
              and then Selected > Top + (List_Rows - 1)
            then
      ...
      begin
         if Selected < Found then
            ...
            Capacity :=
              (if Capacity > Positive'Last / 2 then Positive'Last
               else Capacity * 2);
            Refresh;
            if Selected < Found then
               Selected := Selected + 1;
