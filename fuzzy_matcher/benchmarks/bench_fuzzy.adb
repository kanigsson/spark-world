...
procedure Bench_Fuzzy is
   ...
   Count : constant Positive :=
     (if Ada.Command_Line.Argument_Count > 0
      then Positive'Value (Ada.Command_Line.Argument (1)) else 100_000);
   Query : constant String :=
     (if Ada.Command_Line.Argument_Count > 1 then Ada.Command_Line.Argument (2) else "fma");
   Capacity : constant Natural :=
     (if Ada.Command_Line.Argument_Count > 2
      then Natural'Value (Ada.Command_Line.Argument (3)) else 30);
...
begin
   ...
   for Trial in 1 .. Repeats loop
      ...
      for R in 1 .. Found loop
         Checksum := Checksum + Long_Long_Integer (Results (R).Candidate)
           + Long_Long_Integer (Results (R).Score);
      end loop;
   end loop;
   Elapsed := To_Duration (Clock - Start);
   Ada.Text_IO.Put_Line ("candidates=" & Count'Image & " width=50 K=" & Capacity'Image & " repeats=20 query=" & Query);
   Ada.Text_IO.Put_Line ("seconds=" & Elapsed'Image & " checksum=" & Checksum'Image);
end Bench_Fuzzy;
