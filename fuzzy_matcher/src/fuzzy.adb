package body Fuzzy with SPARK_Mode is
   ...
   begin
      for I in 1 .. Pattern'Length loop
         ...
         pragma Loop_Invariant
           (Previous = Match_End (Pattern, Data, Text, I - 1));
         pragma Loop_Invariant
           (if I > 1 then Previous > 0 and then
              Previous <= Positions (Positions'First + (I - 2)));
         ...
         pragma Assert
           (Offset > 0 and then Offset <= Positions (Positions'First + (I - 1)));
         Previous := Offset;
      end loop;
   ...
   function Word_Start (Previous, Current : Character) return Boolean is
     (Previous = '_' or else Previous = '-' or else Previous = '.'
      or else Previous = ' '
   ...
   with Global => null, Pre => Valid (Data, Text),
     Post => Evaluate'Result.Matched = Is_Subsequence (Pattern, Data, Text)
   ...
   begin
      ...
      for I in 1 .. Pattern'Length loop
         ...
         pragma Loop_Invariant
           (Previous = Match_End (Pattern, Data, Text, I - 1));
         pragma Loop_Invariant
           (Total >= -Long_Long_Integer (Text.Length)
              - 2 * Long_Long_Integer (Previous));
         pragma Loop_Invariant
           (Total <= 60 * Long_Long_Integer (I - 1));
         Offset := Next_Position
           (Data, Text, Previous, Pattern (Pattern'First + (I - 1)));
         ...
         if Offset = 1 or else
           Separator (Character_At (Data, Text, Offset - 1))
         then
            Bonus := Bonus + 16;
         end if;
         if Offset > 1 and then
           Word_Start (Character_At (Data, Text, Offset - 1),
         ...
         Total := Total - 2 * Long_Long_Integer (Offset - Previous - 1) + Bonus;
         Previous := Offset;
      end loop;
      return (True, Score_Type (Total));
   end Evaluate;
   ...
   function Score_Of
     (Pattern : String; Data : String; Text : Text_Slice) return Score_Type is
     (Evaluate (Pattern, Data, Text).Value);
      ...
      for I in 1 .. Pattern'Length loop
         ...
         pragma Loop_Invariant
           (for all P in Positions'Range =>
              (if P - Positions'First < I - 1 then
                 Positions (P) = Match_End
               ...
               pragma Loop_Invariant
                 (for all R in Results'Range =>
                    (if R - Results'First >= Slot
                     and then R - Results'First < Result_Count then
                       Better (Item, Results (R), Candidates)));
               exit when not Better
                 (Item, Results (Results'First + (Slot - 1)), Candidates);
               Slot := Slot - 1;
            end loop;
            if Slot < Results'Length then
               declare
                  Before : constant Search_Result_Array := Results with Ghost;
               begin
                  if Slot > 0 then
                     pragma Assert
                       (Better (Results (Results'First + (Slot - 1)), Item,
                                Candidates));
                  end if;
                  ...
                  for J in reverse Slot + 1 .. Result_Count - 1 loop
                     pragma Loop_Invariant
                       (for all R in Results'Range =>
                          (if R - Results'First <= J then Results (R) = Before (R)
                           elsif R - Results'First < Result_Count then
                             Results (R) = Before (R - 1)));
