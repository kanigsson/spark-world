package Fuzzy with SPARK_Mode, Pure is
   ...
   type Score_Type is range
     -3 * Long_Long_Integer (Integer'Last) ..
   ...
   function Valid (Data : String; Text : Text_Slice) return Boolean is
     (Text.Length = 0
   ...
   function Valid (Data : String; Candidates : Candidate_Array)
                  return Boolean is
     (for all C of Candidates => Valid (Data, C.Text));

   function Character_At
     (Data : String; Text : Text_Slice; Offset : Positive) return Character is
     (Data (Text.First + (Offset - 1)))
   ...
   with Global => null,
     ...
     Post => Next_Position'Result <= Text.Length
       and then
         (if Next_Position'Result > 0 then
            Next_Position'Result > After
            and then Character_At (Data, Text, Next_Position'Result) = Ch)
       and then
         (for all J in 1 .. Text.Length =>
            (if J > After
             and then (Next_Position'Result = 0 or else J < Next_Position'Result)
   ...
   with Ghost, Global => null,
     ...
     Post => Match_End'Result <= Text.Length
       and then (if Count = 0 then Match_End'Result = 0
         else Match_End'Result =
           (if Count > 1 and then
                Match_End (Pattern, Data, Text, Count - 1) = 0
            then 0
            else Next_Position
              (Data, Text, Match_End (Pattern, Data, Text, Count - 1),
               Pattern (Pattern'First + (Count - 1)))))
       and then
         (if Count > 0 and then Match_End'Result > 0 then
            ...
            and then
              (if Count > 1 then
                 Match_End (Pattern, Data, Text, Count - 1) > 0
                 and then Match_End (Pattern, Data, Text, Count - 1) <
                   Match_End'Result)),
     Subprogram_Variant => (Decreases => Count);

   function Is_Subsequence
     (Pattern : String; Data : String; Text : Text_Slice) return Boolean is
     (Pattern'Length = 0 or else
        Match_End (Pattern, Data, Text, Pattern'Length) > 0)
   with Ghost, Pre => Valid (Data, Text);

   function Is_Witness
     (Pattern : String; Data : String; Text : Text_Slice;
      Positions : Match_Position_Array) return Boolean is
     (Positions'Length >= Pattern'Length
      and then (for all P in Positions'Range =>
        (if P - Positions'First < Pattern'Length then
           Positions (P) <= Text.Length
           and then Character_At (Data, Text, Positions (P)) =
             Pattern (Pattern'First + (P - Positions'First))
           and then (if P > Positions'First then
   ...
   with Ghost, Global => null, Always_Terminates,
     Pre => Valid (Data, Text)
       and then Is_Witness (Pattern, Data, Text, Positions),
   ...
   procedure Score
     (Pattern : String; Data : String; Candidate : Text_Slice;
      Matched : out Boolean; Value : out Score_Type)
   with Global => null, Always_Terminates,
     Pre => Valid (Data, Candidate),
     Post => Matched = Is_Subsequence (Pattern, Data, Candidate)
       and then Value = Score_Of (Pattern, Data, Candidate)
       and then (if not Matched then Value = 0);

   function Better
     (Left, Right : Search_Result; Candidates : Candidate_Array)
      return Boolean is
     (Left.Score > Right.Score
      or else (Left.Score = Right.Score
               and then
                 (Candidates (Left.Candidate).Text.Length <
                    Candidates (Right.Candidate).Text.Length
                  or else
                    (Candidates (Left.Candidate).Text.Length =
                       Candidates (Right.Candidate).Text.Length
                     and then Left.Candidate < Right.Candidate))))
   with Pre => Left.Candidate in Candidates'Range
     and then Right.Candidate in Candidates'Range;

   procedure Search
     (Pattern : String; Data : String; Candidates : Candidate_Array;
      Results : out Search_Result_Array; Result_Count : out Natural)
   with Global => null, Always_Terminates,
     Pre => Valid (Data, Candidates),
     Post => Result_Count <= Results'Length
       and then Result_Count <= Candidates'Length
       and then
         (for all R in Results'Range =>
            (if R - Results'First < Result_Count then
               Results (R).Candidate in Candidates'Range
               and then Is_Subsequence
                 (Pattern, Data, Candidates (Results (R).Candidate).Text)
               and then Results (R).Score = Score_Of
                 (Pattern, Data, Candidates (Results (R).Candidate).Text)))
       and then
         (for all R in Results'Range =>
   ...
   procedure Match_Details
     (Pattern : String; Data : String; Candidate : Text_Slice;
      Matched : out Boolean; Value : out Score_Type;
      Positions : out Match_Position_Array; Position_Count : out Natural)
   with Global => null, Always_Terminates,
     Pre => Valid (Data, Candidate) and then Positions'Length >= Pattern'Length,
     Post => Matched = Is_Subsequence (Pattern, Data, Candidate)
       and then Value = Score_Of (Pattern, Data, Candidate)
       and then Position_Count = (if Matched then Pattern'Length else 0)
       and then
         (for all P in Positions'Range =>
            (if P - Positions'First < Position_Count then
               Positions (P) <= Candidate.Length
               and then Character_At (Data, Candidate, Positions (P)) =
                 Pattern (Pattern'First + (P - Positions'First))
               and then
                 (if P > Positions'First then
                    Positions (P - 1) < Positions (P))));
end Fuzzy;
