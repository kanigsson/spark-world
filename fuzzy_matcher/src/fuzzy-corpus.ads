...
package Fuzzy.Corpus
  with SPARK_Mode, Pure
is

   --  Characters still available after the first Used ones.
   function Room (Buffer : String; Used : Natural) return Natural
   is (Buffer'Length - Used)
   ...
   function Appended_Slice
     (Buffer : String; Used : Natural; Item : String) return Text_Slice
   is (First  => (if Item'Length = 0 then 1 else Buffer'First + Used),
       Length => Item'Length)
   with
     Pre => Used <= Buffer'Length and then Item'Length <= Room (Buffer, Used);
   ...
   function Holds
     (Buffer : String; Slice : Text_Slice; Item : String) return Boolean
   is (Slice.Length = Item'Length
       and then (for all K in 1 .. Slice.Length =>
                   Character_At (Buffer, Slice, K)
                   = Item (Item'First + (K - 1))))
   ...
   with
     ...
     Post   =>
       Ok = (Item'Length <= Buffer'Length - Used'Old)
       ...
       and then (if Ok
                 then
                   ...
                   and then Buffer
                              (Buffer'First .. Buffer'First + (Used'Old - 1))
                            = Buffer'Old
                                (Buffer'First .. Buffer'First + (Used'Old - 1))
                 else
                   Used = Used'Old
