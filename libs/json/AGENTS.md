...
## The corpus is reproducible; keep it that way

Every case is either generated by the driver or read from `tests/fixtures/`,
which is committed. The count does not move between machines or runs, so a
change in it means a change in the test, and is worth explaining.

It used to read real documents out of build trees instead — this project's own
`obj/gnatprove/` and, across projects, `inflate/obj/gnatprove/`. That made the
count depend on what had been proved lately and where another project lived,
and most of it silently matched nothing (GNATprove's per-unit files are
dotfiles, which `glob` skips, and it no longer writes `result.json` at all).
Do not reintroduce a build-tree path here. If a new kind of real document is
worth covering, commit it as a fixture.
