...
begin
   ...
   Begin_Capture;
   New_Frame;
   Check (End_Capture = ESC & "[2J" & ESC & "[H", "New_Frame clears and homes");

   ------------------------------------------------------------------ truecolor
   declare
      Out_S : constant String :=
        Blit_One ((Glyph => 'X', Foreground => (Kind => RGB, R => 10, G => 20, B => 30),
                   others => <>), Truecolor);
   begin
      Check (Contains (Out_S, "38;2;10;20;30"), "Truecolor fg -> 38;2;r;g;b");
      Check (Contains (Out_S, "X"),             "glyph emitted");
      Check (Out_S (Out_S'Last - 2 .. Out_S'Last) = ESC & "[0m"
             or else Contains (Out_S, ESC & "[0m"), "frame ends with SGR reset");
   end;

   --  Palette index passes through unchanged at truecolor.
   Check (Contains (Blit_One ((Glyph => ' ',
            Foreground => (Kind => Palette, Index => 200), others => <>), Truecolor),
            "38;5;200"),
          "Palette fg -> 38;5;idx");

   ------------------------------------------------------------------ 256 downgrade
   --  Pure red maps to the top of the 6x6x6 cube: 16 + 36*5 = 196.
   Check (Contains (Blit_One ((Glyph => ' ', Foreground => Red_RGB, others => <>),
            Palette_256), "38;5;196"),
   ...
   declare
      Out_S : constant String :=
        Blit_One ((Glyph => ' ', Foreground => Red_RGB,
                   Attributes => (Bold => True, others => False), others => <>),
                  Monochrome);
   begin
      Check (not Contains (Out_S, "38;"), "Monochrome emits no colour");
      Check (Contains (Out_S, ESC & "[0;1m"), "Monochrome keeps attributes (bold)");
   end;
   ...
   begin
      ...
      begin
         Check (Contains (Got, ESC & "[4;7H"), "Apply positions the changed cell");
