...
is
   ...
   function In_Bounds
     (S : Surface; R : Row_Index; C : Col_Index) return Boolean
   ...
   with
     ...
     Post =>
       ...
       and then (for all RR in Row_Index range 1 .. S.Rows =>
   ...
   with
     Post =>
       Blank'Result.Rows = Rows
       and then Blank'Result.Cols = Cols
       and then (for all R in Row_Index range 1 .. Rows =>
   ...
   with
     Post =>
       (for all R in Row_Index range 1 .. S.Rows =>
          (for all C in Col_Index range 1 .. S.Cols => Get (S, R, C) = To));
   ...
   with
     Pre  =>
       Natural (At_Row) - 1 + Natural (Src.Rows) <= Natural (Dst.Rows)
       and then Natural (At_Col) - 1 + Natural (Src.Cols)
                <= Natural (Dst.Cols),
