...
is
   ...
   function Byte_Offset
     (Line : Tui.Text.Buffer; Column : Natural; End_Point : Boolean)
      return Tui.Text.Byte_Count
   with Global => null, Post => Byte_Offset'Result <= Line'Length
   is
   begin
      ...
      begin
         while Pos <= Hi loop
            ...
            begin
               ...
               W :=
                 (if CP = 16#09#
                  then 8 - Natural (Col mod 8)
                  else Natural (Tui.Width.Char_Width (CP)));
               if Column < Col or else Column - Col < Natural'Max (1, W) then
   ...
   procedure Append
     (Text : in out Payload; B : Tui.Text.Byte; Truncated : in out Boolean)
   with Global => null
   is
   begin
   ...
   with Global => null, Pre => From <= To and then To <= Line'Length
   is
   begin
      if From < To then
      ...
      for N in First.Line .. Last.Line loop
         declare
            ...
            From : constant Tui.Text.Byte_Count :=
              (if N = First.Line
               then Byte_Offset (Line, First.Col, False)
               else 0);
            To   : constant Tui.Text.Byte_Count :=
              (if N = Last.Line
               then Byte_Offset (Line, Last.Col, True)
               else Line'Length);
         begin
   ...
   begin
      return
        (1 => Alphabet (A / 4 + 1),
         ...
         3 =>
           (if I + 1 <= P.Len
            then Alphabet ((B mod 16) * 4 + C / 64 + 1)
            else '='),
         4 => (if I + 2 <= P.Len then Alphabet (C mod 64 + 1) else '='));
   end Encode;
