...
is
   ...
   type Sel_Move is
     (Sel_Up,
      Sel_Down,            --  one entry
      Sel_Page_Up,
      Sel_Page_Down,  --  a page of entries
      Sel_Top,
      Sel_Bottom);        --  ends of the list
   ...
   with Global => null, Pre => Total > 0, Post => Selected <= Total;
   ...
   with
     ...
     Post   => --  Nothing moves for a line that was already on screen.
       ...
       and then (if Eng.Height (E'Old) > 0
                 then
                   Line >= Eng.Top_Line (E)
