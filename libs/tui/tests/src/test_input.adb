...
begin
   ...
   Check
     (N = 1
      and then E.Kind = Char
      and then E.Code = Character'Pos ('a')
      and then not E.Mods.Ctrl
      and then not E.Mods.Alt,
      "'a' -> Char a");
   ...
   Check
     (N = 1
      and then E.Kind = Char
      and then E.Code = Character'Pos ('C')
      and then E.Mods.Ctrl,
      "0x03 -> Ctrl-C");
   ...
   Check
     (N = 1
      and then E.Kind = Char
      and then E.Code = Character'Pos ('x')
      and then E.Mods.Alt
      and then not E.Mods.Ctrl,
      "ESC x -> Alt-x");
   ...
   Check
     (N = 1 and then E.Kind = Char and then E.Code = 16#E9#,
      "UTF-8 C3 A9 -> U+00E9");

   Decode ((16#E2#, 16#82#, 16#AC#), False, E, N);  --  '€' U+20AC
   Check
     (N = 1 and then E.Kind = Char and then E.Code = 16#20AC#,
   ...
   Decode
     ((16#1B#,
      ...
      N);
   Check
     (N = 1 and then E.Kind = Wheel_Up and then E.Col = 80 and then E.Row = 24,
   ...
   Decode
     ((16#1B#,
      ...
      N);
   Check
     (N = 1 and then E.Kind = Wheel_Down and then E.Col = 1 and then E.Row = 1,
   ...
   Decode
     ((16#1B#, C ('['), C ('<'), C ('1'), C (';'), C ('5'), C ('A')),
      False,
      E,
      N);
   Check
     (N = 1 and then E.Kind = Unknown, "ESC[<1;5A -> Unknown (not Ctrl-Up)");
   ...
   Decode
     ((16#1B#,
      ...
      N);
   Check
     (N = 2 and then E.Kind = Mouse_Press,
   ...
   begin
      ...
      Check
        (Is_Pending (D) and then not Avail, "after ESC: pending, no event");
      Feed (D, C ('['), Ev, Avail);
      Feed (D, C ('B'), Ev, Avail);
      Check
        (not Is_Pending (D) and then Avail and then Ev.Kind = Down,
