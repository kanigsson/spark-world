...
is
   ...
   with
     Post =>
       ...
       and then (case Encoding_Width (Value) is
                   when 1 => Octet_At (Encode_One'Result, 0) = Natural (Value),
                   when 2 =>
                     Octet_At (Encode_One'Result, 0)
                     = 16#C0# + Natural (Value) / 64
                     and then Octet_At (Encode_One'Result, 1)
                   ...
                   when 3 =>
                     ...
                     and then Octet_At (Encode_One'Result, 2)
                              = 16#80# + Natural (Value) mod 64,
                   when 4 =>
                     Octet_At (Encode_One'Result, 0)
                     ...
                     and then Octet_At (Encode_One'Result, 2)
                              = 16#80# + (Natural (Value) / 64) mod 64
                     and then Octet_At (Encode_One'Result, 3)
                              = 16#80# + Natural (Value) mod 64);
   ...
   with
     ...
     Post =>
       ...
       and then (if Sequence_Width_At'Result > 0
   ...
   with
     ...
     Post =>
       ...
       and then (case Decode_One'Result.Width is
                   when 1 =>
                     Natural (Decode_One'Result.Value)
                     = Octet_At (S, Byte_Offset),
                   when 2 =>
                     Natural (Decode_One'Result.Value)
   ...
   is (S'Length = 0
       or else (for all Offset in Natural range 0 .. S'Length - 1 =>
   ...
   with
     ...
     Post  =>
       (Static =>
          ...
          and then Scalar_Sequences.Length (Model'Result)
   ...
   is (Count = 0
       or else (Same_Bytes (Left, Left_Offset, Right, Right_Offset, Count - 1)
                and then Octet_At (Left, Left_Offset + Count - 1)
   ...
   is (if Count = 0 then "" else S (S'First .. S'Last - (S'Length - Count)))
   with Ghost => Static, Pre => Count <= S'Length;

   function Is_Byte_Concatenation (Left, Right, Result : String) return Boolean
   is (Result'Length = Left'Length + Right'Length
       and then Same_Bytes (Left, 0, Result, 0, Left'Length)
       and then Same_Bytes (Right, 0, Result, Left'Length, Right'Length))
   with Ghost => Static, Pre => Left'Length <= Natural'Last - Right'Length;
   ...
   with
     ...
     Global             => null,
     Pre                =>
       Offset <= S'Length and then Count <= S'Length - Offset,
   ...
   with
     ...
     Pre                =>
       ...
       and then Second_Count <= Left'Length - Left_Offset - First_Count
       and then Right_Offset <= Right'Length
       and then First_Count <= Right'Length - Right_Offset
       and then Second_Count <= Right'Length - Right_Offset - First_Count
       and then Same_Bytes
     ...
     Post               =>
       Same_Bytes
         (Left, Left_Offset, Right, Right_Offset, First_Count + Second_Count),
   ...
   procedure Lemma_Encoding_Injective (Left, Right : String; Value : Text)
   with
     Ghost  => Static,
     Global => null,
     Pre    =>
   ...
   with
     ...
     Post   =>
       ...
       and then Is_Concatenation (Model (Left), Model (Right), Model (Result));
     ...
     Post =>
       (Runtime => Code_Point_Length'Result <= S'Length,
        Static  =>
          ...
          and then To_Big_Integer (S'Length)
   ...
   with
     ...
     Global => null,
     Pre    => Is_ASCII (S),
     Post   => Is_Valid_UTF_8 (S) and then Code_Point_Length (S) = S'Length;

   function Element (S : String; Index : Positive) return Scalar_Value
   with
     Pre  => Is_Valid_UTF_8 (S) and then Index <= Code_Point_Length (S),
     Post =>
       (Static =>
          Element'Result
   ...
   with
     ...
     Post => Code_Point_Length'Result <= Span.Past_Last - Span.First;
   ...
   with
     ...
     Post =>
       (Runtime =>
          ...
          and then Code_Point_Length (Slice'Result)
                   = Code_Point_Length (S, Span)
          and then Is_Valid_UTF_8 (Slice'Result),
        Static  =>
          Same_Bytes (S, Span.First, Slice'Result, 0, Slice'Result'Length)
   ...
   with
     ...
     Post =>
       (Runtime =>
          ...
          and then Code_Point_Length (S, To_Byte_Span'Result) = Count,
        Static  =>
          Is_Slice
            (Source => Model (S),
   ...
   with
     Pre  => Is_Valid_UTF_8 (S) and then From - 1 <= Code_Point_Length (S),
     Post =>
   ...
   is (Is_Valid_UTF_8 (Source)
       and then Big_Split_Model_Index (State)
                <= Scalar_Sequences.Length (Model (Source)) + 1
       and then (if Split_Complete (State)
                 then
   ...
   with
     ...
     Post =>
       (Runtime =>
          ...
          and then (if Has_Value then Is_Valid_Byte_Span (Source, Segment)),
        Static  =>
          Is_Valid_Split_State (Source, State)
          and then (if Has_Value
                    then
                      Is_Split_Step
                        (Source     => Model (Source),
   ...
   with
     Pre  =>
       (Runtime => Is_Valid_UTF_8 (S), Static => Is_Valid_Cursor (S, Cursor)),
     Post =>
       (Runtime => Has_Element'Result = (Byte_Offset (Cursor) < S'Length),
        Static  =>
          Has_Element'Result
   ...
   with
     Pre  =>
       (Runtime => Is_Valid_UTF_8 (S) and then Byte_Offset (Cursor) < S'Length,
     ...
     Post =>
       (Runtime =>
          ...
          and then Value = Decode_One (S, Byte_Offset (Cursor'Old)).Value
   ...
   with
     Post =>
       ...
       and then (if Validate'Result.Valid
   ...
   with
     ...
     Post   =>
       ...
       and then Decode_One (Encode_One (Value), 0).Width
                = Encoding_Width (Value)
