...
is
   ...
   is (Byte_Offset = S'Length
       or else (Valid_At (S, Byte_Offset)
                and then Valid_From
                           (S,
   ...
   is (if Byte_Offset = S'Length
       then 0
         ...
         1 + Length_From (S, Byte_Offset + Sequence_Width_At (S, Byte_Offset)))
   with
     Pre                =>
       Byte_Offset <= S'Length and then Valid_From (S, Byte_Offset),
     Post               =>
       (Runtime => Length_From'Result <= S'Length - Byte_Offset,
        Static  =>
          ...
          and then To_Big_Integer (S'Length - Byte_Offset)
                   <= 4 * To_Big_Integer (Length_From'Result)),
     Subprogram_Variant => (Decreases => S'Length - Byte_Offset);

   function Element_From
     (S : String; Byte_Offset : Natural; Index : Positive) return Scalar_Value
   is (if Index = 1
       then Decode_One (S, Byte_Offset).Value
       else
         Element_From
           (S, Byte_Offset + Sequence_Width_At (S, Byte_Offset), Index - 1))
   with
     Pre                =>
   ...
   procedure Lemma_ASCII_Valid_From (S : String; Byte_Offset : Natural)
   with
     Ghost              => Static,
     Global             => null,
     Pre                =>
       Byte_Offset <= S'Length
       and then (Byte_Offset = S'Length
                 or else (for all Offset in
                            Natural range Byte_Offset .. S'Length - 1 =>
   ...
   function Element (S : String; Index : Positive) return Scalar_Value
   is (Element_From (S, 0, Index));

   function Cursor_At_Index (S : String; Index : Positive) return Cursor_Type
   with
     Pre  => Is_Valid_UTF_8 (S) and then Index - 1 <= Code_Point_Length (S),
     Post =>
       (Runtime => Model_Index (Cursor_At_Index'Result) = Cursor_Index (Index),
   ...
   begin
      for Position in 1 .. Index - 1 loop
         ...
         pragma
           Loop_Invariant (Model_Index (Cursor) = Cursor_Index (Position));
         Next (S, Cursor, Value);
   ...
   is (First = Past_Last
       or else (Valid_At (S, First)
                and then Sequence_Width_At (S, First) <= Past_Last - First
                and then Is_Valid_Range
                           (S,
                            First + Sequence_Width_At (S, First),
                            Past_Last)))
   with
   ...
   with
     ...
     Post               => Valid_Range_Length'Result <= Past_Last - First,
     Subprogram_Variant => (Decreases => Past_Last - First);

   function Range_Model (S : String; First, Past_Last : Natural) return Text
   is (if First = Past_Last
       then Scalar_Sequences.Empty_Sequence
       else
         Scalar_Sequences.Add
           (Range_Model (S, First + Sequence_Width_At (S, First), Past_Last),
            1,
      ...
      if First < Old_Past_Last then
         ...
         begin
            Lemma_Extend_Valid_Range
              (S, First + Width, Old_Past_Last, New_Past_Last);
   ...
   with
     ...
     Pre                =>
       ...
       and then Is_Valid_Range (Left, Left_Offset, Left_Offset + Count),
     Post               =>
       Valid_From (Right, Right_Offset)
       and then Scalar_Sequences.Length (Model_From (Right, Right_Offset))
                = To_Big_Integer
                    (Valid_Range_Length
                       (Left, Left_Offset, Left_Offset + Count))
       and then Model_From (Right, Right_Offset)
   ...
   begin
      if Span.First = Span.Past_Last then
         ...
         begin
            ...
            Lemma_Copied_Range (S, Span.First, Result, 0, Result'Length);
      ...
      for Position in 1 .. Count loop
         ...
         pragma
           Loop_Invariant
             (Static =>
                Is_Valid_Range
                  (S, Byte_Offset (Start_Cursor), Byte_Offset (End_Cursor)));
         pragma
           Loop_Invariant
             (Static =>
                Valid_Range_Length
         ...
         begin
            ...
            Lemma_Extend_Slice
              (Source => Model (S),
               First  => To_Big_Integer (First),
               Count  => To_Big_Integer (Position - 1),
               Before => Range_Model (S, Byte_Offset (Start_Cursor), Old_End),
               Value  => Value,
               After  =>
                 Range_Model
                   (S, Byte_Offset (Start_Cursor), Byte_Offset (End_Cursor)));
   ...
   function Slice (S : String; First : Positive; Count : Natural) return String
   is (Slice (S, To_Byte_Span (S, First, Count)));

   function Find
     (Haystack : String; Needle : String; From : Positive := 1) return Natural
   with
     Refined_Post =>
       (Static =>
          Is_First_Occurrence
      ...
      with
        ...
        Post =>
          (Static =>
             ...
             = Matches_At
                 (Model (Haystack), Model (Needle), To_Big_Integer (First)))
      is
      begin
         ...
         for Offset in 1 .. Needle_Length loop
            pragma Loop_Invariant (First - 1 + Offset <= Haystack_Length);
            pragma
              Loop_Invariant
                (Static =>
             ...
             (Static =>
                (for all I in Model (Haystack) =>
                   (if I >= To_Big_Integer (From)
                    ...
                    then
                      not Matches_At (Model (Haystack), Model (Needle), I))));
   ...
   begin
      ...
      if Source_Length = 0 or else State.Index > Cursor_Index (Source_Length)
      then
         pragma
           Assert
             (Static =>
      ...
      begin
         if Found = 0 then
            Segment :=
              To_Byte_Span
                (Source, First => From, Count => Source_Length - (From - 1));
            State.Index := Cursor_Index (Source_Length) + 1;
            State.Done := True;
         else
            pragma Assert (Found >= From);
            pragma Assert (Found - 1 + Separator_Length <= Source_Length);
            Segment :=
              To_Byte_Span (Source, First => From, Count => Found - From);
            State.Index :=
              Cursor_Index (Found) + Cursor_Index (Separator_Length);
         end if;
      end;
   end Next;
   ...
   is (Is_Valid_UTF_8 (S)
       ...
       and then Cursor_Index_Conversions.To_Big_Integer (Cursor.Index)
                <= Scalar_Sequences.Length (Model (S)) + 1
       and then Scalar_Sequences.Length (Model_From (S, Cursor.Offset))
   ...
   begin
      ...
      begin
         while Has_Element (Prefix, Prefix_Cursor) loop
            ...
            pragma
              Loop_Invariant (Static => Is_Valid_Cursor (Whole, Whole_Cursor));
            pragma
              Loop_Invariant
            ...
            pragma
              Loop_Variant
                (Decreases => Prefix'Length - Byte_Offset (Prefix_Cursor));
                   ...
                   (for all I in Model (Suffix) =>
                      (if I
                       ...
                       then
                         ...
                         = Scalar_Sequences.Get
                             (Model (Whole),
                              To_Big_Integer (Difference) + I))));
            pragma
              Loop_Variant
                (Decreases => Suffix'Length - Byte_Offset (Suffix_Cursor));
   ...
   begin
      ...
      loop
         ...
         pragma
           Loop_Invariant (Static => Is_Valid_Cursor (Right, Right_Cursor));
         pragma
           Loop_Invariant
         ...
         pragma
           Loop_Variant (Decreases => Left'Length - Byte_Offset (Left_Cursor));
   ...
   with
     ...
     Post   =>
       ...
       and then (for all I in
                   Natural
                     range 0 .. Natural (Decode_One (S, Offset).Width) - 1 =>
                   Octet_At (S, Offset + I)
   ...
   begin
      case Unit.Width is
         when 1 =>
            ...
            pragma
              Assert (Static => Octet_At (S, Offset) = Octet_At (Encoded, 0));
         ...
         when 2 =>
            ...
            pragma
              Assert
                (Static => Octet_At (S, Offset + 1) = Octet_At (Encoded, 1));
            pragma Assert (Static => S (S'First + Offset) = Encoded (1));
         ...
         when 3 =>
            ...
            pragma
              Assert
                (Static => Octet_At (S, Offset + 2) = Octet_At (Encoded, 2));
            pragma Assert (Static => S (S'First + Offset) = Encoded (1));
            pragma Assert (Static => S (S'First + Offset + 1) = Encoded (2));
         ...
         when 4 =>
            ...
            pragma
              Assert
                (Static => Octet_At (S, Offset + 3) = Octet_At (Encoded, 3));
            pragma Assert (Static => S (S'First + Offset) = Encoded (1));
            pragma Assert (Static => S (S'First + Offset + 1) = Encoded (2));
            pragma Assert (Static => S (S'First + Offset + 2) = Encoded (3));
   ...
   with
     ...
     Pre    =>
       ...
       and then Decode_One (Left, Left_Offset).Value
   ...
   is
      Left_Unit  : constant Decoded_Unit := Decode_One (Left, Left_Offset)
      with Ghost => Static;
      Right_Unit : constant Decoded_Unit := Decode_One (Right, Right_Offset)
      with Ghost => Static;
   begin
      ...
      case Left_Unit.Width is
         when 1 =>
            pragma
              Assert
                (Static =>
                   Same_Bytes (Left, Left_Offset, Right, Right_Offset, 1));

         when 2 =>
            pragma
              Assert
                (Static =>
                   Same_Bytes (Left, Left_Offset, Right, Right_Offset, 2));

         when 3 =>
            pragma
              Assert
                (Static =>
                   Same_Bytes (Left, Left_Offset, Right, Right_Offset, 3));

         when 4 =>
            pragma
              Assert
                (Static =>
                   Same_Bytes (Left, Left_Offset, Right, Right_Offset, 4));
      end case;
   end Lemma_Equal_Decoded_Bytes;

   procedure Lemma_Same_Bytes_Reflexive (S : String; Offset, Count : Natural)
   is
   begin
      if Count > 0 then
   ...
   begin
      if Index < Count - 1 then
         Lemma_Same_Bytes_At
           (Left, Left_Offset, Right, Right_Offset, Count - 1, Index);
      end if;
   ...
   begin
      if Dropped < Count then
         Lemma_Same_Bytes_Suffix
           (Left, Left_Offset, Right, Right_Offset, Count - 1, Dropped);
         Lemma_Same_Bytes_At
           (Left, Left_Offset, Right, Right_Offset, Count, Count - 1);
      end if;
   ...
   begin
      if Count > 0 then
         Lemma_Transfer_To_Prefixes (Left, Right, Prefix_Count, Count - 1);
      end if;
   end Lemma_Transfer_To_Prefixes;

   procedure Lemma_Same_Bytes_Prefixes (Left, Right : String; Count : Natural)
   is
   begin
      Lemma_Transfer_To_Prefixes (Left, Right, Count, Count);
   end Lemma_Same_Bytes_Prefixes;

   procedure Lemma_Same_Bytes_Whole_Equality (Left, Right : String) is
   begin
   ...
   begin
      ...
      pragma
        Assert (Static => Octet_At (Left, Offset) = Octet_At (Right, Offset));
      if Width >= 2 then
         pragma
           Assert
             (Static =>
      ...
      pragma
        Assert
          (Static => Decode_One (Left, Offset) = Decode_One (Right, Offset));
   ...
   begin
      if Offset < Left'Length then
         ...
         begin
            pragma
              Assert (Static => Width = Sequence_Width_At (Right, Offset));
   ...
   with
     ...
     Post   =>
       ...
       and then Decode_One (Left, Left_Offset)
   ...
   begin
      ...
      Lemma_Same_Bytes_At (Left, Left_Offset, Right, Right_Offset, Count, 0);
      if Width >= 2 then
         Lemma_Same_Bytes_At
           (Left, Left_Offset, Right, Right_Offset, Count, 1);
      end if;
      ...
      pragma
        Assert (Static => Sequence_Width_At (Right, Right_Offset) = Width);
      pragma
        Assert
          (Static =>
   ...
   begin
      if Count > 0 then
         ...
         begin
            ...
            Lemma_Same_Bytes_Suffix
              (Left, Left_Offset, Right, Right_Offset, Count, Width);
            Lemma_Rebased_Valid_Range
              (Left,
               Left_Offset + Width,
               Right,
   ...
   with
     ...
     Post               =>
       Valid_From (Right, Right_Offset)
       and then Model_From (Left, Left_Offset)
      ...
      if Left_Offset < Left'Length then
         ...
         begin
            ...
            Lemma_Rebased_Model_From
              (Left, Left_Offset + Width, Right, Right_Offset + Width);
         end;
      end if;
   ...
   begin
      if Offset = Left'Length then
         Lemma_Rebased_Model_From (Right, 0, Result, Left'Length);
         pragma
           Assert
             (Static =>
                Scalar_Sequences.Length (Model_From (Left, Offset)) = 0);
         pragma Assert (Static => Model_From (Result, Offset) = Model (Right));
         pragma
           Assert
             (Static =>
                Is_Concatenation
         ...
         declare
            ...
            with Ghost => Static;
         begin
            Lemma_Same_Bytes_Suffix (Left, 0, Result, 0, Left'Length, Offset);
            Lemma_Same_Bytes_Decode (Left, Offset, Result, Offset, Count);
            Lemma_Concatenation_From (Left, Right, Result, Next_Offset);
            Lemma_Prepend_Concatenation
   ...
   procedure Lemma_Encoding_Injective (Left, Right : String; Value : Text) is
      Left_Cursor  : Cursor_Type := First (Left);
      Right_Cursor : Cursor_Type := First (Right);
      Left_Value   : Scalar_Value;
      Right_Value  : Scalar_Value;
   begin
      ...
      while Has_Element (Left, Left_Cursor) loop
         ...
         pragma
           Loop_Invariant
             (Static =>
                Same_Bytes (Left, 0, Right, 0, Byte_Offset (Left_Cursor)));
         pragma
           Loop_Variant (Decreases => Left'Length - Byte_Offset (Left_Cursor));
         ...
         begin
            pragma
              Assert (Static => Same_Bytes (Left, 0, Right, 0, Old_Offset));
            Next (Left, Left_Cursor, Left_Value);
            Next (Right, Right_Cursor, Right_Value);
   ...
   begin
      if Left = Right then
         ...
         pragma
           Assert (Static => (Left = Right) = (Model (Left) = Model (Right)));
         return;
      end if;
   ...
   procedure Lemma_Concatenation (Left, Right, Result : String) is
   begin
