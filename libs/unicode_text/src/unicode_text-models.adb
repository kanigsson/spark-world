...
is
   ...
   procedure Lemma_Extend_Slice
     (Source : Text;
      ...
      After  : Text) is
   begin
      pragma Assert (Static => Scalar_Sequences.Length (After) = Count + 1);
      pragma
        Assert
          (Static =>
             (for all I in After =>
                (if I <= Count
                 then
                   ...
                   and then Scalar_Sequences.Get (Before, I)
             ...
             (for all I in First_Result =>
                (if I <= Scalar_Sequences.Length (Left)
                 then
                   ...
                   and then Scalar_Sequences.Get (Second_Result, I)
                            = Scalar_Sequences.Get (Left, I)
                 else
                   Scalar_Sequences.Get (First_Result, I)
             ...
             (for all I in Middle_Right =>
                (if I <= Scalar_Sequences.Length (Middle)
                 then
                   ...
                   and then Scalar_Sequences.Get (Middle_Right, I)
                            = Scalar_Sequences.Get (Middle, I)
                 else
                   Scalar_Sequences.Get
                     (Left_Grouped_Result, Scalar_Sequences.Length (Left) + I)
                   ...
                   and then Scalar_Sequences.Get (Middle_Right, I)
                            = Scalar_Sequences.Get
                                (Right,
                                 I - Scalar_Sequences.Length (Middle)))));
      pragma
        Assert
          (Static =>
   ...
   begin
      pragma
        Assert
          (Static =>
             Scalar_Sequences.Length (New_Result)
             ...
             (for all I in New_Left =>
                (if I = 1
                 then
                   ...
                   and then Scalar_Sequences.Get (New_Left, I)
