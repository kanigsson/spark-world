...
is
   ...
   is ((Scalar_Sequences.Length (Left) < Scalar_Sequences.Length (Right)
       ...
       or else (for some I in Left =>
                  ...
                  and then Scalar_Sequences.Get (Left, I)
   ...
   is (Scalar_Sequences.Length (After) = Scalar_Sequences.Length (Before) + 1
       ...
       and then Scalar_Sequences.Get (After, Scalar_Sequences.Last (After))
                = Value)
   with Ghost => Static;
   ...
   is (Scalar_Sequences.Length (Result)
       ...
       and then (for all I in Right =>
                   Scalar_Sequences.Get
                     (Result, Scalar_Sequences.Length (Left) + I)
                   = Scalar_Sequences.Get (Right, I)))
   with Ghost => Static;
   ...
   is (Scalar_Sequences.Length (Needle) = 0
       or else (for some First in Haystack =>
                  Is_Slice
                    (Source => Haystack,
                     First  => First,
   ...
   is (From <= Scalar_Sequences.Length (Haystack) + 1
       and then (if Result = 0
                 then
                   Scalar_Sequences.Length (Needle) > 0
                   and then (for all I in Haystack =>
                               (if I >= From
   ...
   is (From <= Scalar_Sequences.Length (Source) + 1
       and then (if Result = 0
                 then
                   ...
                   and then (for all I in Source =>
                               (if I >= From and then I < Result
                                then
                                  Scalar_Sequences.Get (Source, I) /= Value))))
   with Ghost => Static;

   function Is_Last_Occurrence
     (Source : Text; Value : Scalar_Value; Result : Big_Natural) return Boolean
   is (if Result = 0
       then (for all I in Source => Scalar_Sequences.Get (Source, I) /= Value)
       else
         Result >= 1
         and then Result <= Scalar_Sequences.Length (Source)
         and then Scalar_Sequences.Get (Source, Result) = Value
   ...
   is (Scalar_Sequences.Length (Separator) > 0
       and then Is_Slice
                  (Source => Source,
                   ...
                   Result => Segment)
       and then Is_Delimiter_Free
                  (Source, Separator, First, Scalar_Sequences.Length (Segment))
       and then (if Final
                 then
                   Next_First = Scalar_Sequences.Length (Source) + 1
                   and then First + Scalar_Sequences.Length (Segment)
                            = Next_First
                 else
                   Matches_At
                     (Source,
