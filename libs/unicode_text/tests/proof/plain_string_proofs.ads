...
is
   ...
   procedure Equal_Active_Prefixes (Left, Right : String; Used : Natural)
   with
     Ghost  => Static,
     Global => null,
     Pre    =>
       ...
       and then Unicode_Text.UTF_8.Same_Bytes (Left, 0, Right, 0, Used),
     Post   =>
       Unicode_Text.UTF_8.Prefix_Bytes (Left, Used)
       = Unicode_Text.UTF_8.Prefix_Bytes (Right, Used)
       and then Unicode_Text.UTF_8.Model
                  (Unicode_Text.UTF_8.Prefix_Bytes (Left, Used))
                = Unicode_Text.UTF_8.Model
                    (Unicode_Text.UTF_8.Prefix_Bytes (Right, Used));

   procedure Append_Valid_String (Before, Appended, After : String)
   with
     Ghost  => Static,
     Global => null,
     Pre    =>
       ...
       and then Unicode_Text.UTF_8.Is_Byte_Concatenation
                  (Before, Appended, After),
     Post   =>
       Unicode_Text.UTF_8.Is_Valid_UTF_8 (After)
       and then Unicode_Text.Models.Is_Concatenation
   ...
   with
     ...
     Pre    =>
       ...
       and then From - 1 <= Unicode_Text.UTF_8.Code_Point_Length (Haystack),
     Post   =>
