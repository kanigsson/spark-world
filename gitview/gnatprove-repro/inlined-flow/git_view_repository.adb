...
package body Git_View_Repository with SPARK_Mode => Off is
   ...
   procedure Load (V : M.View_State; F : in out Frame) is
      ...
      begin
         for I in 1 .. G.File_Count (Cached_Changes) loop
            if (G.Has_Path (Cached_Changes, I, G.New_Side)
                and then G.Path (Cached_Changes, I, G.New_Side) = Path)
              or else (G.Has_Path (Cached_Changes, I, G.Old_Side)
                and then G.Path (Cached_Changes, I, G.Old_Side) = Path)
      ...
      begin
         case V.Kind is
            when M.Commit =>
               return Git (Args ("show") + (To_String (Target) & ":" & Path));
