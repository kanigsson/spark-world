...
with tempfile.TemporaryDirectory(prefix="gitview-explorer-") as tmp:
    ...
    f = probe(tree="changed_ancestors")
    check("context/unchanged.txt" not in f["tree"] and "src/\n" in f["tree"], "changed files and ancestors")
    check("src/\n" not in probe(tree="changed_only")["tree"], "flat changed-only tree")
    f = probe(lens="plain")
