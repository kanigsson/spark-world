...
procedure Explorer_Probe is
   ...
   begin
      Put_Line ("=== " & Name);
      for B of D.Bytes loop Put (Character'Val (B)); end loop;
      New_Line;
   end Dump;
begin
   ...
   if Argument_Count >= 6 then V.Base := To_Text (Argument (6)); V.Automatic_Base := V.Base.Last = 0; end if;
   if Argument_Count >= 7 then V.Repository_Search := To_Text (Argument (7)); end if;
   if Argument_Count >= 8 then V.Path_Filter := To_Text (Argument (8)); end if;
   if Argument_Count >= 9 then Git_View_Source.Make_Revision (Argument (9), V.History_Root, Ok); end if;
