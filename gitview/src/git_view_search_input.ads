--  Git_View_Search_Input — the git viewer's search-pattern editor, as proved
--  SPARK.
--
...
--  fixed-size buffer carries it, bringing the edit logic — including the
--  UTF-8 encoding of a typed code point — into SPARK, proved free of
--  run-time errors. This mirrors the standalone pager's editor; once a third
--  consumer appears the unit belongs in a shared app-kit crate.
--
...
package Git_View_Search_Input with SPARK_Mode => On is

   --  Mirror the engine's cap: there is no point holding bytes the engine
   --  would drop on Set_Pattern.
   ...
   --  The pattern bytes (UTF-8), 1-based — ready for the engine's
   --  Set_Pattern and for the host's status-line painter.
