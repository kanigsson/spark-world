...
package Git_View_Explorer with
  SPARK_Mode => On, Abstract_State => State, Initializes => State
is
   procedure Init
   ...
   procedure On_Key
     (Event : Tui.Input.Key_Event; Dirty, Quit : out Boolean)
     with Global => (In_Out => (State, Git_View_Repository.State));
   procedure Tick (Dirty : out Boolean)
     with Global => (In_Out => (State, Git_View_Repository.State));
   procedure Stop with Global => (In_Out => (State, Git_View_Repository.State));
