...
package Git_View_Repository
  with SPARK_Mode => On, Abstract_State => State, Initializes => State
is
   package M renames Git_View_Model;
   use type Tui.Text.Doc_Ref;
   --  A row identity is a slice of its pane's name buffer rather than a
   --  text of its own: a whole-tree frame then costs the paths themselves,
   --  and no path is too long to name.
   ...
   --  names nothing rather than raising.
   function Named (Names : Name_Ref; R : Row_Target) return String
   is (if Names = null
         or else R.First not in Names'Range
         or else R.Last not in R.First - 1 .. Names'Last
       then ""
       else Names (R.First .. R.Last));
   function Commit_Id (F : Frame; Row : Tui.Text.Line_Number) return String
   is (if F.Commits = null or else Row not in F.Commits'Range
       then ""
       else Named (F.Commit_Names, F.Commits (Row)));
   function Path (F : Frame; Row : Tui.Text.Line_Number) return String
   is (if F.Paths = null or else Row not in F.Paths'Range
       then ""
       else Named (F.Path_Names, F.Paths (Row)));
   function Loaded (F : Frame) return Boolean
   is (F.History /= null and then F.Tree /= null and then F.Source /= null);
