...
is
   ...
   function Image (S : Text) return String
   is (S.Data (1 .. S.Last));
   ...
   function Row_Kind (Id : String) return Snapshot_Kind
   is (if Id = Worktree_Row
       then Worktree
       elsif Id = Index_Row
       then Staging
       else Commit);
   ...
   function Row_Snapshot (Id : String) return String
   is (if Row_Kind (Id) = Commit then Id else "");
   ...
   function Is_Message (Path : String) return Boolean
   is (Path = Message_Row);
   ...
   function In_Range (Line, First, Count : Natural) return Boolean
   is (Line >= First and then Line - First < Count);
   function Deletion_Anchor (First, Count : Natural) return Natural
   is (if Count = 0 and then First < Natural'Last then First + 1 else First);
   function In_Context
