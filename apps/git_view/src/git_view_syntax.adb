...
package body Git_View_Syntax
  with SPARK_Mode => On
is

   use type Tui.Text.Byte;

   function Lower (B : Tui.Text.Byte) return Tui.Text.Byte
   is (if B in Character'Pos ('A') .. Character'Pos ('Z')
       then B + (Character'Pos ('a') - Character'Pos ('A'))
       else B);
   ...
   function Ends_With (Line : Tui.Text.Buffer; Suffix : String) return Boolean
   ...
   begin
      ...
      Found := True;
      if Ends_With (Line, ".adb")
        or else Ends_With (Line, ".ads")
        or else Ends_With (Line, ".ada")
        or else Ends_With (Line, ".gpr")
      then
         Lang := Ada_Lang;
      elsif Ends_With (Line, ".c")
        or else Ends_With (Line, ".h")
        or else Ends_With (Line, ".cc")
        or else Ends_With (Line, ".hh")
        or else Ends_With (Line, ".cpp")
        or else Ends_With (Line, ".hpp")
        or else Ends_With (Line, ".cxx")
        or else Ends_With (Line, ".rs")
        or else Ends_With (Line, ".go")
        or else Ends_With (Line, ".java")
        or else Ends_With (Line, ".js")
        or else Ends_With (Line, ".jsx")
        or else Ends_With (Line, ".ts")
        or else Ends_With (Line, ".tsx")
        or else Ends_With (Line, ".swift")
        or else Ends_With (Line, ".kt")
      then
         Lang := C_Family;
      elsif Ends_With (Line, ".py")
        or else Ends_With (Line, ".pyi")
      ...
      then
         Lang := Shell_Like;
      elsif Ends_With (Line, ".json")
        or else Ends_With (Line, ".toml")
        or else Ends_With (Line, ".yaml")
        or else Ends_With (Line, ".yml")
      then
         Lang := Config;
      end if;
   end Header_Language;
   ...
   is (B in Character'Pos ('a') .. Character'Pos ('z')
   ...
   is (Is_Identifier_Start (B) or else Is_Digit (B));
   ...
   begin
      ...
      begin
         while Remaining > 0 and then Col < Tui.Pager.Max_Dim loop
            ...
            pragma Loop_Invariant (Remaining <= Line'Last - Pos + 1);
