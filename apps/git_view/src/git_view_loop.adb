         ...
         begin
            ...
            declare
               Rows               : constant Row_Count :=
                 Row_Count (Integer'Min (Size.Rows, Max_Extent));
               Cols               : constant Col_Count :=
                 Col_Count (Integer'Min (Size.Cols, Max_Extent));
               Current            : Surface := Blank (Rows, Cols);
               Previous           : Surface := Blank (Rows, Cols);
               Changes            :
                 Tui.Surface.Diff.Change_Array
                   (1 .. Tui.Surface.Diff.Cell_Count (Current));
               Count              : Natural;
            ...
            begin
               ...
               loop
                  ...
                  if Dirty or else Ready then
                     Clear (Current);
                     Git_View_Explorer.Paint (Current);
                     Tui.Surface.Diff.Compute
                       (Previous, Current, Changes, Count);
