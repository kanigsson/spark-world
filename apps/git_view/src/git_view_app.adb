...
is
   ...
   is (List_Doc = null and then Diff_Doc = null);

   -------------------
   -- Has_Documents --
   -------------------

   function Has_Documents return Boolean
   is (List_Doc /= null and then Diff_Doc /= null);
   ...
   with Global => (Input => (List_Doc, Selected)), Pre => List_Doc /= null
   is
   ...
   procedure Tint_Row
     (S : in out Surface; R : Row_Index; Fg : Color; Bold : Boolean := False)
   with Global => null, Pre => R <= S.Rows
   is
   begin
   ...
   procedure Shade_Row (S : in out Surface; R : Row_Index; Bg : Color)
   with Global => null, Pre => R <= S.Rows
   is
   begin
   ...
   with
     Global => null,
     Pre    =>
       R <= S.Rows and then From in Line'Range and then To in From .. Line'Last
   is
   ...
   with
     Global => (Input => (Diff_Doc, Diff_Eng, Syntax_Enabled)),
     Pre    => Diff_Doc /= null
   is
      Total       : constant Tui.Text.Line_Total :=
        Tui.Text.Line_Count (Diff_Doc.all.Idx);
      Top         : constant Tui.Text.Line_Number :=
        Eng_Pkg.Top_Line (Diff_Eng);
      Lang        : Syn.Language :=
        (if Total > 0 and then Top <= Total
         then Syn.Language_At (Diff_Doc.all.Bytes, Diff_Doc.all.Idx, Top)
         else Syn.Plain);
      Found       : Boolean;
   ...
   with Global => (Input => (List_Doc, List_Eng)), Pre => List_Doc /= null
   is
      Total : constant Tui.Text.Line_Total :=
        Tui.Text.Line_Count (List_Doc.all.Idx);
   ...
   begin
      ...
      for R in Row_Index range 1 .. LS.Rows loop
         ...
         begin
            exit when LN > Total;
            Git_View_Sha.Extract
              (List_Doc.all.Bytes, List_Doc.all.Idx, LN, Id);
            if Git_View_Sha.Valid (Id) then
               ...
               declare
                  Line     : constant Tui.Text.Buffer :=
                    Tui.Text.Line (List_Doc.all.Idx, List_Doc.all.Bytes, LN);
                  Found    : Boolean;
   ...
   procedure Highlight_Selection
     (LS : in out Surface; Total : Tui.Text.Line_Total)
   ...
   procedure Highlight_Text_Selection
     (PS : in out Surface; Pane : Pol.Pane; E : Eng_Pkg.Instance)
   ...
   begin
      ...
      Mark.Overlay
        (PS, Eng_Pkg.Top_Line (E), Eng_Pkg.Left_Col (E), First, Last);
   ...
   is
      ...
      Accent : constant Color :=
        (if Searching
         then Thm.Prompt_Accent
         elsif Note /= Git_View_Status.No_Note
         then Thm.Note_Accent
         else Thm.Bar_Accent);
   ...
   begin
      ...
      if Searching then
         Git_View_Status.Format_Prompt (L, Forward, Edit.Bytes (Pattern));
   ...
   begin
      ...
      begin
            ...
            begin
               ...
               Copy
                 (LS,
                  S,
                  At_Row => 1,
                  At_Col => Col_Index (Places (1).Start_Col));
            end;
         end if;
         ...
         if Diff_Cols > 0 then
            ...
            begin
               Eng_Pkg.Render
                 (Diff_Eng, DS, Diff_Doc.all.Bytes, Diff_Doc.all.Idx);
               Colorize_Diff (DS);
   ...
   with
     Global =>
       (In_Out => (List_Eng, Diff_Eng, Selected, Note),
   ...
   begin
      if Focused = Pol.List_Pane then
         Eng_Pkg.Handle
           (List_Eng, Cmd, List_Doc.all.Bytes, List_Doc.all.Idx, Res);
         declare
            Total : constant Tui.Text.Line_Total :=
              Tui.Text.Line_Count (List_Doc.all.Idx);
         begin
   ...
   procedure Jump_Diff
     (Target : Nav.Landmark; Forward_Jump : Boolean; Changed : out Boolean)
   with
     Global => (In_Out => Diff_Eng, Input => Diff_Doc),
     Pre    => Diff_Doc /= null
   is
      Total : constant Tui.Text.Line_Total :=
        Tui.Text.Line_Count (Diff_Doc.all.Idx);
   ...
   with
     Global =>
       (In_Out => (Diff_Doc, Diff_Eng, Diff_Id, Selected, Note),
        Input  => List_Doc),
     Pre    => List_Doc /= null and then Diff_Doc /= null,
     Post   => Diff_Doc /= null
   is
   ...
   begin
      ...
      Git_View_Sha.Extract
        (List_Doc.all.Bytes, List_Doc.all.Idx, Selected, Id);
      if Git_View_Sha.Valid (Id) then
   ...
   with
     Global => (In_Out => (List_Eng, Selected), Input => List_Doc),
     Pre    => List_Doc /= null
   is
      Total : constant Tui.Text.Line_Total :=
        Tui.Text.Line_Count (List_Doc.all.Idx);
   ...
   procedure Handle_Search_Key
     (Event : Key_Event; Dirty : out Boolean; Quit : out Boolean)
   with
     Global =>
       (In_Out => (List_Eng, Diff_Eng, Selected, Searching, Pattern, Note),
   ...
   begin
      ...
      case Event.Kind is
         when Enter     =>
            Searching := False;
            if Focused = Pol.List_Pane then
               Eng_Pkg.Set_Pattern (List_Eng, Edit.Bytes (Pattern));
            else
               Eng_Pkg.Set_Pattern (Diff_Eng, Edit.Bytes (Pattern));
            end if;
            declare
               Ignore : Boolean;
            begin
   ...
   with
     Global => (Input => (List_Doc, Diff_Doc, List_Eng, Diff_Eng)),
     Pre    =>
       List_Doc /= null
       and then Diff_Doc /= null
       and then F'First = 1
       and then F'Last = 2,
     Post   =>
       F (1).Total = Tui.Text.Line_Count (List_Doc.all.Idx)
   ...
   with
     Global => (Input => (List_Doc, Diff_Doc), In_Out => (Note, Mouse)),
     Pre    =>
       List_Doc /= null
       and then Diff_Doc /= null
   ...
   is
      ...
      Total     : constant Tui.Text.Line_Total :=
        (if In_List
         then Tui.Text.Line_Count (List_Doc.all.Idx)
   ...
   with
     ...
     Pre    =>
       List_Doc /= null and then Diff_Doc /= null and then G.Kind = Gest.Wheel
   is
      In_List : constant Boolean := Pol.Named (G.Pane) = Pol.List_Pane;
      Cmd     : constant Eng_Pkg.Command :=
   ...
   begin
      ...
      if Event.Kind
         in Mouse_Press | Mouse_Release | Mouse_Motion | Wheel_Up | Wheel_Down
      then
