...
is
   ...
   function Source_After (E : Edit) return Natural
   is (E.Source + (if E.Kind = Insert then 0 else 1));
   function Target_After (E : Edit) return Natural
   is (E.Target + (if E.Kind = Delete then 0 else 1));
   function Output_Length (S : Script) return Natural
   is (if S'Length = 0 then 0 else Target_After (S (S'Last)));

   --  Complete, ordered consumption, exact expected source values, and
   --  contiguous output. No fuzzy matching or partial application.
   function Valid (A : Sequence; S : Script) return Boolean
   is (A'First = 1
       and then A'Length <= Max_Length
       and then S'First = 1
       and then S'Length <= 2 * Max_Length
       and then (if S'Length = 0
                 then A'Length = 0
                 else
                   S (1).Source = 0
                   and then S (1).Target = 0
                   and then Source_After (S (S'Last)) = A'Length)
       and then (for all I in S'Range =>
                   S (I).Source <= A'Length
                   and then (if S (I).Kind /= Insert
                             then
                               S (I).Source < A'Length
                               and then S (I).Value = A (S (I).Source + 1))
                   and then Target_After (S (I)) <= Output_Length (S)
                   and then (if S (I).Kind /= Delete
                             then S (I).Target < Max_Length)
                   and then (if I < S'Last
                             then
                               Source_After (S (I)) = S (I + 1).Source
                               and then Target_After (S (I))
                                        = S (I + 1).Target)));

   function Describes (A, B : Sequence; S : Script) return Boolean
   is (Valid (A, S)
       and then B'First = 1
   ...
   with
     Global => null,
     Pre    => Valid (A, S),
     Post   => Apply'Result'First = 1 and then Describes (A, Apply'Result, S);
   ...
   with
     ...
     Global             => null,
     Pre                =>
       S'First = 1 and then S'Length <= 2 * Max_Length and then N <= S'Length,
   ...
   with
     ...
     Post   =>
       Edit_Cost'Result = Prefix_Cost (S, S'Length)
       and then Edit_Cost'Result <= S'Length;

   function Workspace_Shape (W : Workspace) return Boolean
   is (W'First (1) = 0
       and then W'Last (1) in 0 .. Max_Length
       and then W'First (2) = -W'Last (1)
       and then W'Last (2) = W'Last (1));
   function Frontier_Values (A : Sequence; W : Workspace) return Boolean
   is (for all R in W'Range (1) =>
   ...
   function Delete_Bound (N : Count; V : Integer) return Integer
   is (if V < 0 or else N = 0 then -1 else Integer'Min (V + 1, N))
   with Pre => V in -1 .. N;
   function Insert_Bound (M : Count; K, V : Integer) return Integer
   is (if V < 0 or else M = 0 then -1 else Integer'Min (V, M + K))
   with Pre => K in -Max_Length .. Max_Length and then V in -1 .. Max_Length;

   function Closed (A, B : Sequence; K, V : Integer) return Boolean
   is (V = -1
       or else (V in 0 .. A'Length
                and then V - K in 0 .. B'Length
                and then (if V < A'Length and then V - K < B'Length
                          then A (V + 1) /= B (V - K + 1))))
   with
     Pre =>
       A'First = 1
       ...
       and then K in -Max_Length .. Max_Length
       and then V in -1 .. Max_Length;

   function Certificate_Cell
     (A, B : Sequence; W : Workspace; R : Natural; K : Integer) return Boolean
   is (Closed (A, B, K, W (R, K))
       and then (if K = A'Length - B'Length then W (R, K) < A'Length)
       and then (if R = 0
                 then W (R, K) >= 0
                 else
                   (if K > -R
                      and then Delete_Bound (A'Length, W (R - 1, K - 1))
                               in Integer'Max (0, K)
                                .. Integer'Min (A'Length, B'Length + K)
                    then W (R, K) >= Delete_Bound (A'Length, W (R - 1, K - 1)))
                   and then (if K < R
                               and then Insert_Bound
                                          (B'Length, K, W (R - 1, K + 1))
                                        in Integer'Max (0, K)
                                         .. Integer'Min
                                              (A'Length, B'Length + K)
                             then
   ...
   function Lower_Bound
     (A, B : Sequence; W : Workspace; D : Natural) return Boolean
   is (A'First = 1
       and then B'First = 1
       and then A'Length <= Max_Length
       and then B'Length <= Max_Length
   ...
   with
     ...
     Pre    =>
       ...
       and then B'Length <= Max_Length
       and then S'First = 1
       and then S'Length = A'Length + B'Length
       and then Work'First (1) = 0
       and then Work'Last (1) in 0 .. Max_Length
     ...
     Post   =>
       ...
       and then (if Minimal
                 then Lower_Bound (A, B, Work, Edit_Cost (S (1 .. Last))));
