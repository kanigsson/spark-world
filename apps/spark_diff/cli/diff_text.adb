...
package body Diff_Text is
   ...
   is
      ...
      procedure Read_Stream (Input : access Ada.Streams.Root_Stream_Type'Class)
      is
         use Ada.Streams;
            ...
            for I in 1 .. Natural (Last) loop
               ...
               if Bytes (I) = ASCII.NUL then
                  raise Constraint_Error
                    with "NUL byte in input; only text files are supported";
               end if;
   ...
   begin
      while Cursor <= S'Last loop
         if S (Cursor).Kind = Keep then
            ...
            First :=
              Natural'Max (1, Cursor - Natural'Min (Context, Cursor - 1));
