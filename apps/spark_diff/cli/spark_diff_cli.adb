...
procedure Spark_Diff_Cli is
   ...
   begin
      ...
      Put_Line
        ("Compare text files by exact line bytes; '-' reads stdin once.");
      ...
      Put_Line
        ("  --label TEXT        old/new header label, repeated at most twice");
      Put_Line ("  --max-distance N    Myers budget (0..4096; default 256)");
      Put_Line ("  --help              show this help");
      Put_Line ("  --                  end options");
      Put_Line
        ("Exit: 0 identical, 1 different, 2 trouble. NUL input is rejected.");
      Put_Line
        ("Budget exhaustion emits a whole-file replacement. No patch command.");
   end Help;
      ...
      for C of Value loop
         ...
         if N > Limit then
            raise Constraint_Error
              with "numeric option exceeds limit: " & Value;
         end if;
      end loop;
      return N;
   end Number;
   ...
   begin
      for C of Value loop
         ...
         then
            raise Constraint_Error
              with "header labels cannot contain tabs, newlines, or NUL";
         end if;
      end loop;
      if Value'Length = 0 then
   ...
   while Arg <= Argument_Count loop
      ...
      begin
         if not Options_Done and then A = "--" then
            ...
            Help;
            return;
         elsif not Options_Done and then (A = "-u" or else A = "--unified")
         then
            null;
         ...
         then
            ...
            raise Constraint_Error
              with "unknown option: " & A & "; see --help";
         else
            if Path_Count = 2 then
               raise Constraint_Error
                 with "expected exactly two input paths; see --help";
            end if;
