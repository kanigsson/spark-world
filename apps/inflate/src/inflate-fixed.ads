...
is
   ...
   function Code_Length (B : Byte) return Code_Bit_Length
   is (if B <= 143 then 8 else 9);

   function Code (B : Byte) return Natural
   is (if B <= 143 then 48 + Natural (B) else 256 + Natural (B))
   ...
   type Symbol_Kind is (Literal, Match, End_Of_Block, Other, Truncated);
   ...
   function Match_Applies
     (Data : Byte_Array; Position, Length, Distance : Natural) return Boolean
   is (Position <= Data'Length
   ...
   with
     ...
     Post =>
       Selected_Token'Result.Kind in Literal | Match
       and then Selected_Token'Result.Position in Position + 1 .. Data'Length
       and then (if Selected_Token'Result.Kind = Literal
                 then
                   Selected_Token'Result.Value = Data (Data'First + Position)
                   ...
                   Selected_Token'Result.Length in 3 .. 10
   ...
   with
     Pre  => Data'Length <= Max_Input and then Position < Data'Length,
     Post =>
       Next_Position'Result in Position + 1 .. Data'Length
   ...
   is (if Selected_Token (Data, Position).Kind = Match
       then 12
   ...
   function Plan_Remaining (Data : Byte_Array; Count : Natural) return Natural
   is (if Count = 0
       then 0
   ...
   function Token_Boundary (Data : Byte_Array; Count : Natural) return Boolean
   is (Plan_Remaining (Data, Count) = 0)
   with Ghost, Pre => Data'Length <= Max_Input and then Count <= Data'Length;
   ...
   --  token's start.
   function Plan_Start (Data : Byte_Array; Count : Natural) return Natural
   is (if Count = 0
       then 0
       elsif Token_Boundary (Data, Count - 1)
       then Count - 1
   ...
   with
     ...
     Pre                =>
       Data'Length <= Max_Input and then Count <= Data'Length,
     Post               =>
       Plan_Start'Result <= Count
       and then (if Count > 0
                 then
                   Plan_Start'Result < Count
                   and then Token_Boundary (Data, Plan_Start'Result)
                   and then Count <= Next_Position (Data, Plan_Start'Result)
                   and then (if Token_Boundary (Data, Count)
                             then
   ...
   function Data_Bits (Data : Byte_Array; Count : Natural) return Natural
   is (if Count = 0
       then 0
       elsif Token_Boundary (Data, Count)
       then
         Data_Bits (Data, Plan_Start (Data, Count))
   ...
   procedure Lemma_Encoding_Next (Data : Byte_Array; Index : Natural)
   with
     Ghost,
     Pre  =>
     ...
     Post =>
       Token_Boundary (Data, Next_Position (Data, Index))
       and then Plan_Start (Data, Next_Position (Data, Index)) = Index
       and then Data_Bits (Data, Next_Position (Data, Index))
                = Data_Bits (Data, Index) + Token_Bit_Cost (Data, Index)
       and then (for all Count in
                   Index + 1 .. Next_Position (Data, Index) - 1 =>
                   not Token_Boundary (Data, Count)
                   and then Data_Bits (Data, Count) = Data_Bits (Data, Index));
   ...
   with
     ...
     Post =>
       Encoded_Bit_Count'Result = Data_Bits (Data, Data'Length)
       and then Encoded_Bit_Count'Result <= 9 * Data'Length;
   pragma Assertion_Policy (Post => Check);

   --  Exact raw-DEFLATE size for Data, including selected matches.
   pragma Assertion_Policy (Post => Ignore);
   function Encoded_Size (Data : Byte_Array) return Positive
   is ((Encoded_Bit_Count (Data) + 17) / 8)
   with
     Pre  => Data'Length <= Max_Input,
     Post =>
       Encoded_Size'Result <= Max_Size (Data'Length)
   ...
   function Bit_Value (Input : Byte_Array; Position : Natural) return Natural
   is (Bit_Cursors.Bit_Value (Input, Position, Bit_Cursors.Lsb_First))
   with
     Pre  =>
       Input'Length <= Max_Stream_Bytes and then Position < 8 * Input'Length,
   ...
   function Prefix_Value
     (Input : Byte_Array; Start : Natural; Length : Natural) return Natural
   with
     Pre                =>
   ...
   function Fixed_Header (Input : Byte_Array) return Boolean
   is (Input'Length > 0
   ...
   is (3 + Data_Bits (Data, Count) <= 8 * Input'Length
       and then (for all I in 0 .. Count - 1 =>
                   (if Token_Boundary (Data, I)
                      and then Selected_Token (Data, I).Kind = Match
                    then
                      3 + Data_Bits (Data, I) + 12 <= 8 * Input'Length
                      and then Prefix_Value (Input, 3 + Data_Bits (Data, I), 7)
   ...
   with
     ...
     Post =>
       ...
       and then (if Next_Symbol'Result.Kind /= Truncated
                 then Next_Symbol'Result.Position > Position)
       and then (if Next_Symbol'Result.Kind = Literal
                 then
                   Next_Symbol'Result.Length = 1
       ...
       and then (if Next_Symbol'Result.Kind = Match
                 then
                   Next_Symbol'Result.Length in 3 .. 10
       ...
       and then (if Next_Symbol'Result.Kind = End_Of_Block
                 then
                   Next_Symbol'Result.Length = 0
                   and then Next_Symbol'Result.Position = Position + 7
                   and then Prefix_Value (Input, Position, 7) = 0)
       and then (if 7 <= 8 * Input'Length - Position
                   and then Prefix_Value (Input, Position, 7) = 0
                 then
                   Next_Symbol'Result = (End_Of_Block, 0, 0, 0, Position + 7))
       and then (if 12 <= 8 * Input'Length - Position
                 ...
                 then
                   ...
                   = (Match,
                      ...
                      Position + 12))
       and then (if 8 <= 8 * Input'Length - Position
                 ...
                 then
                   ...
                   = (Literal,
                      ...
                      0,
                      Position + 8))
       and then (if 9 <= 8 * Input'Length - Position
     ...
     Contract_Cases     =>
       (Next_Symbol (Input, Position).Position > 8 * Consumed =>
        ...
        and then Next_Symbol (Input, Position).Kind = Literal =>
          Spec_Matches'Result
          = (Next_Symbol (Input, Position).Value = Data (Data'First + Index)
   ...
   with
     ...
     Post =>
       Encoding_Matches'Result = Spec_Matches (Input, Consumed, Data, 3, 0);
   pragma Assertion_Policy (Post => Check);
   ...
   with
     ...
     Contract_Cases     =>
       (Next_Symbol (Input, Position).Kind = Literal
        ...
        and then Next_Symbol (Input, Position).Length <= Max_Input - Decoded =>
          (if Spec_Walk
                (Input,
                 Next_Symbol (Input, Position).Position,
        ...
        Next_Symbol (Input, Position).Kind = End_Of_Block                    =>
          Spec_Walk'Result = (True, Next_Symbol (Input, Position).Position, 0),
        others                                                               =>
          Spec_Walk'Result = (False, 0, 0)),
   ...
   with
     ...
     Post =>
       (if Fixed_Header (Input) and then Walk (Input, 3, 0).Valid
   ...
   with
     Ghost,
     Pre  =>
       Is_Encoding (Before, Consumed, Data)
   ...
   with
     Ghost,
     Pre  =>
       Is_Encoding (Input, Consumed, Left)
   ...
   with
     ...
     Post   =>
       Produced = Encoded_Size (Input)
       and then Is_Encoding (Output, Produced, Input);
   ...
   with
     ...
     Post   =>
       ...
       and then (if Analyze (Input).Valid
                   and then Analyze (Input).Decoded_Length <= Output'Length
                 then
                   Success
                   and then Consumed = (Analyze (Input).End_Bit + 7) / 8
