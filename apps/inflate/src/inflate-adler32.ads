...
is
   ...
   with
     Global => null,
     Post   =>
       Word32 (Initial_State'Result.A) = (Adler and 16#FFFF#) mod Base
   ...
   with
     ...
     Post   =>
       ...
       and then Model_Byte_Step'Result.B = S.B + S.A + Component (Value);

   function Pack (S : State) return Word32
   with
     Global => null,
     Post   => Pack'Result = (Shift_Left (Word32 (S.B), 16) or Word32 (S.A));
