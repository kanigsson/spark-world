...
is
   ...
   --  The table above is the power it tabulates. The table is what the Kraft
   --  sums are proved with, because a concrete array is what lets the provers
   --  case-split where the exponent is a code length computed at run time, and
   --  a bound written against it has to reach the contracts of anything stated
   --  as a power — Ore's field operations among them. Enumerating the widths is
   --  the only way to say it, and saying it here beside the table is what keeps
   --  the enumeration to one place.
   procedure Lemma_Pow2_Power (Length : Natural)
   with Ghost, Pre => Length <= 15, Post => Pow2 (Length) = 2**Length;

   type Codebook_Kind is (Fixed_Literal_Length, Fixed_Distance, Canonical);
   ...
   Fixed_Distance_Book       : constant Codebook := (Kind => Fixed_Distance);
   ...
   is (if Count = 0
       then 0
         ...
         Count_Length (Lengths, Length, Count - 1)
   ...
   function First_Code
     (Counts : Length_Count_Array; Length : Code_Length_Pos) return Natural
   is (if Length = 1
       then 0
       else 2 * (First_Code (Counts, Length - 1) + Counts (Length - 1)))
   with
     Post               =>
       First_Code'Result <= Max_Symbols * (Pow2 (Length) - 2),
     Subprogram_Variant => (Decreases => Length);

   function Kraft_Value
     (Counts : Length_Count_Array; Length : Code_Length) return Natural
   is (if Length = 0
       then 0
       else 2 * Kraft_Value (Counts, Length - 1) + Counts (Length))
   with
     Post               =>
       Kraft_Value'Result <= Max_Symbols * (Pow2 (Length) - 1),
     Subprogram_Variant => (Decreases => Length);

   function Exact_Counts (Book : Codebook) return Boolean
   is (Book.Kind = Canonical
       and then (for all Length in Code_Length_Pos =>
                   Book.Counts (Length)
   ...
   function Code_Of (Book : Codebook; Symbol : Symbol_Index) return Natural
   is (case Book.Kind is
         when Fixed_Literal_Length =>
           (if Symbol <= 143
            then 48 + Symbol
   ...
   function Lengths_At_Most
     (Book : Codebook; Maximum : Code_Length) return Boolean
   is (for all Symbol in Symbol_Index => Length_Of (Book, Symbol) <= Maximum);
   ...
   procedure Lemma_Length_Arrays_Equal (Left, Right : Code_Length_Array)
   with
     Ghost,
     Global => null,
   ...
   with
     ...
     Pre    =>
       Left.Kind = Canonical
       and then Right.Kind = Canonical
       and then (for all I in Symbol_Index =>
   ...
   procedure Lemma_Lengths_At_Most_From_Fields
     (Left, Right : Codebook; Maximum : Code_Length)
   with
     Ghost,
     Global => null,
     Pre    =>
   ...
   procedure Lemma_Code_Of_From_Fields
     (Left, Right : Codebook; Symbol : Symbol_Index)
   with
     Ghost,
     Global => null,
     Pre    =>
