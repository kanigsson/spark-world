...
is
   ...
   with
     ...
     Pre    =>
       ...
       and then Codebooks.Length_Of
                  (Literal_Lengths, Natural (Data (Data'First)))
                > 0
       and then Codebooks.Length_Of (Literal_Lengths, 256) > 0
       and then (for all Symbol in 257 .. 264 =>
                   Codebooks.Length_Of (Literal_Lengths, Symbol) > 0)
       and then (for all Symbol in 0 .. 3 =>
   ...
   procedure Lemma_Byte_Run_Books_Cover
     (Data            : Byte_Array;
      Literal_Lengths : Codebooks.Codebook;
      Distances       : Codebooks.Codebook) is
   begin
                      ...
                      Codebooks.Length_Of
                        (Literal_Lengths,
                         Payload.Length_Symbol
                           (Fixed.Selected_Token (Data, J).Length))
                      > 0
                      and then Codebooks.Length_Of
                                 (Distances,
                                  Payload.Distance_Symbol
                                    (Fixed.Selected_Token (Data, J).Distance))
                               > 0
                    else
                      Codebooks.Length_Of
                        (Literal_Lengths, Natural (Data (Data'First + J)))
                      > 0)));
         pragma Assert (Data (Data'First + I) = Data (Data'First));
               ...
               if Token.Kind = Fixed.Match then
                  ...
                  pragma
                    Assert
                      (Codebooks.Length_Of
                         (Literal_Lengths,
                          Payload.Length_Symbol (Token.Length))
                         > 0);
                  ...
                  pragma
                    Assert
                      (Codebooks.Length_Of
                         (Distances, Payload.Distance_Symbol (Token.Distance))
                         > 0);
               else
                  pragma Assert (Token.Kind = Fixed.Literal);
                  pragma Assert (Token.Value = Data (Data'First));
                  pragma
                    Assert
                      (Codebooks.Length_Of
                         (Literal_Lengths, Natural (Data (Data'First)))
   ...
   function Is_Dummy
     (Frequencies : Frequency_Array; Active : Symbol_Count; I : Symbol_Index)
      return Boolean
   is (if Active = 0
       then I <= 1
       elsif Active = 1
   ...
   procedure Lemma_Active_Monotone
     (Frequencies : Frequency_Array; First, Last : Symbol_Count)
   with
     Ghost,
     Pre                =>
       Frequencies'First = 0
       ...
       and then First <= Last,
     Post               =>
       Active_Count (Frequencies, First) <= Active_Count (Frequencies, Last),
     Subprogram_Variant => (Decreases => Last - First);

   procedure Lemma_Active_Monotone
     (Frequencies : Frequency_Array; First, Last : Symbol_Count) is
   begin
      if First < Last then
   ...
   procedure Lemma_Dummy_Inactive
     (Frequencies : Frequency_Array; Active : Symbol_Count)
   with
     Ghost,
     Pre  =>
       Frequencies'First = 0
       and then Frequencies'Length in 2 .. Max_Symbols
       and then Active = Active_Count (Frequencies, Frequencies'Length),
     Post =>
       (if Active = 0
   ...
   procedure Lemma_Dummy_Inactive
     (Frequencies : Frequency_Array; Active : Symbol_Count) is
   begin
      if Active <= 1 then
         Lemma_Active_Monotone (Frequencies, 2, Frequencies'Length);
         if Active = 0 then
   ...
   with
     ...
     Post =>
       ...
       = Dummy_Count (Frequencies, Active, Count)
         + (if Is_Dummy (Frequencies, Active, Symbol_Index (Count))
            then 1
            else 0);
   ...
   with
     ...
     Pre  => Length in 1 .. 9,
     Post =>
       Pow2 (Length) * Pow2 (15 - Length) = Pow2 (15)
       and then (if Length > 1
   ...
   procedure Lemma_Model_Frame
     (Before, After : Code_Length_Array; Count : Symbol_Count)
   with
     Ghost,
     Pre                =>
       Before'First = 0
       ...
       and then (for all I in 0 .. Count - 1 => Before (I) = After (I)),
     Post               =>
       Assigned_Count (After, Count) = Assigned_Count (Before, Count)
       and then Code_Space (After, Count) = Code_Space (Before, Count),
     Subprogram_Variant => (Decreases => Count);

   procedure Lemma_Model_Frame
     (Before, After : Code_Length_Array; Count : Symbol_Count) is
   begin
      if Count > 0 then
   ...
   with
     ...
     Post =>
       ...
       and then (for all I in Lengths'Range =>
                   (if I /= Position then Lengths (I) = Lengths'Old (I)))
       ...
       and then Code_Space (Lengths, Position + 1)
                = Code_Space (Lengths'Old, Position) + Pow2 (15 - Value)
   is
      Before : constant Code_Length_Array := Lengths
      with Ghost;
   begin
      Lengths (Position) := Value;
   ...
   with
     Pre  =>
       ...
       and then Value <= 1,
     Post =>
       Fixed.Bit_Value (Output, Position) = Value
       and then (for all P in 0 .. 8 * Output'Length - 1 =>
                   (if P /= Position
                    then
   ...
   begin
      ...
      --  frame stated above.
      Bit_Cursors.Set_Bit (Output, Position, Value = 1, Bit_Cursors.Lsb_First);
   end Set_Stream_Bit;
   ...
   procedure Write_Length_Code
     (Output : in out Byte_Array; Start : Natural; Value : Code_Length)
   with
     Pre  =>
       Output'Length <= Fixed.Max_Stream_Bytes
     ...
     Post =>
       Length_Code_At (Output, Start, Value)
   ...
   with
     ...
     Pre  =>
       ...
       and then Header_Encodes (Before, Literal_Lengths, Distances)
       and then (for all Position in 0 .. Header_Bit_Count - 1 =>
      ...
      for I in 0 .. Literal_Length_Count - 1 loop
         ...
         pragma
           Loop_Invariant
             (for all J in 0 .. I =>
                Result (J)
                = Fixed.Prefix_Value (Input, Header_Prefix_Bits + 4 * J, 4));
         pragma
           Loop_Invariant
   ...
   with
     ...
     Post =>
       (for all I in 0 .. Distance_Count - 1 =>
          ...
          = Fixed.Prefix_Value
              (Input, Header_Prefix_Bits + 4 * (Literal_Length_Count + I), 4))
   ...
   procedure Lemma_Length_Code_Value
     (Input : Byte_Array; Start : Natural; Value : Code_Length)
   with
     Ghost,
     Pre  =>
       Input'Length <= Fixed.Max_Stream_Bytes
   ...
   procedure Lemma_Length_Code_Value
     (Input : Byte_Array; Start : Natural; Value : Code_Length) is
   begin
   ...
   with
     ...
     Pre  =>
       Input'Length <= Fixed.Max_Stream_Bytes
       and then Input'Length >= Header_Byte_Count
       and then Books_Encodable (Before_Literals, Before_Distances)
       and then Books_Encodable (After_Literals, After_Distances)
       and then Before_Literals.Lengths = After_Literals.Lengths
       and then Before_Distances.Lengths = After_Distances.Lengths
       and then Header_Encodes (Input, Before_Literals, Before_Distances),
     Post => Header_Encodes (Input, After_Literals, After_Distances);

   procedure Lemma_Header_From_Book_Fields
     (Input                             : Byte_Array;
      Before_Literals, Before_Distances : Codebooks.Codebook;
      After_Literals, After_Distances   : Codebooks.Codebook) is
   begin
      Codebooks.Lemma_Length_Of_From_Fields (Before_Literals, After_Literals);
   ...
   begin
      for I in Codebooks.Symbol_Index loop
         ...
         if I < Literal_Length_Count then
            ...
            pragma Assert (Codebooks.Length_Of (Recovered_Literals, I) = 0);
            pragma Assert (Codebooks.Length_Of (Literal_Lengths, I) = 0);
         end if;
      end loop;
      Codebooks.Lemma_Canonical_Lengths_Equal
        (Recovered_Literals, Literal_Lengths);
      Codebooks.Lemma_Exact_Books_Equal (Recovered_Literals, Literal_Lengths);
      ...
      for I in Codebooks.Symbol_Index loop
         ...
         if I < Distance_Count then
            ...
            pragma Assert (Codebooks.Length_Of (Recovered_Distances, I) = 0);
            pragma Assert (Codebooks.Length_Of (Distances, I) = 0);
         end if;
      end loop;
      Codebooks.Lemma_Canonical_Lengths_Equal (Recovered_Distances, Distances);
      Codebooks.Lemma_Exact_Books_Equal (Recovered_Distances, Distances);
   ...
   begin
      ...
      pragma Assert (Active = Active_Count (Frequencies, Frequencies'Length));
      ...
      while Capacity < Leaves loop
         ...
         pragma Loop_Invariant (if Long_Length > 1 then Capacity / 2 < Leaves);
      ...
      pragma
        Assert
          (Active + Dummy_Count (Frequencies, Active, Frequencies'Length)
             = Leaves);
      ...
      for I in Frequencies'Range loop
         pragma
           Loop_Invariant (Seen = Assigned_Count (Lengths, Symbol_Count (I)));
         pragma
           Loop_Invariant (Short_Seen = Symbol_Count'Min (Seen, Short_Leaves));
         pragma Loop_Invariant (Space_Units = Seen + Short_Seen);
         pragma Loop_Invariant (Space_Units <= Capacity);
         pragma
           Loop_Invariant (Code_Space (Lengths, Symbol_Count (I)) = Space);
         pragma Loop_Invariant (Space = Space_Units * Pow2 (15 - Long_Length));
         pragma Loop_Invariant (Seen <= Leaves);
         pragma Loop_Invariant (Short_Seen <= Short_Leaves);
         pragma
           Loop_Invariant
             (Seen
                = Active_Count (Frequencies, Symbol_Count (I))
         ...
         pragma
           Assert
             (if Is_Dummy (Frequencies, Active, I) then Frequencies (I) = 0);

         if Frequencies (I) > 0 or else Is_Dummy (Frequencies, Active, I) then
            pragma
              Assert
                (Active_Count (Frequencies, Symbol_Count (I) + 1)
                   + Dummy_Count (Frequencies, Active, Symbol_Count (I) + 1)
                   = Seen + 1);
            ...
            pragma
              Assert
                (Code_Space (Lengths, Symbol_Count (I) + 1)
                   = Code_Space (Lengths, Symbol_Count (I)));
            pragma
              Assert (Seen = Assigned_Count (Lengths, Symbol_Count (I) + 1));
            pragma Assert (Code_Space (Lengths, Symbol_Count (I) + 1) = Space);
         end if;
         pragma
           Assert
             (Seen
      ...
      pragma Assert (Space = Space_Units * Pow2 (15 - Long_Length));
      pragma Assert (Code_Space (Lengths, Lengths'Length) = Space);
      pragma Assert (Space = Capacity * Pow2 (15 - Long_Length));
      pragma Assert (Code_Space (Lengths, Lengths'Length) = Pow2 (15));
      pragma Assert (Assigned_Count (Lengths, Lengths'Length) = Leaves);
      pragma
        Assert
   ...
   begin
      ...
      for I in Lengths'Range loop
         pragma
           Loop_Invariant
             (for all J in Lengths'First .. I - 1 => Full (J) = Lengths (J));
         pragma
           Loop_Invariant
             (for all J in I .. Codebooks.Symbol_Index'Last => Full (J) = 0);
         Full (I) := Lengths (I);
      end loop;
   ...
   is
      Initial : constant Byte_Array := Output
      with Ghost;
   begin
         ...
         pragma
           Loop_Invariant
             (for all P in Position .. 8 * Output'Length - 1 =>
                Fixed.Bit_Value (Output, P) = Fixed.Bit_Value (Initial, P));
         Set_Stream_Bit (Output, Position, Expected_Header_Bit (Position));
      end loop;
      ...
      for I in 0 .. Literal_Length_Count - 1 loop
         ...
         pragma
           Loop_Invariant
             (for all P in
                Header_Prefix_Bits + 4 * I .. 8 * Output'Length - 1 =>
                Fixed.Bit_Value (Output, P) = Fixed.Bit_Value (Initial, P));
         Write_Length_Code
           (Output,
            Header_Prefix_Bits + 4 * I,
   ...
   is
      ...
      Body_End   : Natural;
   begin
      Serialize_Header (Literal_Lengths, Distances, Output, Header_End);
      pragma Assert (Header_End = Header_Bit_Count);

      declare
         Header_Output : constant Byte_Array := Output
         with Ghost;
      begin
         Serialize_Payload
           (Data, Literal_Lengths, Distances, Output, Header_End, Body_End);
         pragma
           Assert
   ...
   begin
      ...
      Build_Codebook (Literal_Frequencies, Literal_Lengths, Literals_Ready);
      Build_Codebook
         ...
         pragma
           Assert
             (Codebooks.Length_Of
                (Literal_Lengths, Natural (Data (Data'First)))
                > 0);
         Lemma_Byte_Run_Books_Cover (Data, Literal_Lengths, Distances);
         Serialize_Body (Data, Literal_Lengths, Distances, Output, Produced);
   ...
   is
      ...
      with
        Ghost,
        Pre  =>
          Before_Literals.Kind = Codebooks.Canonical
      ...
      procedure Lemma_Substitute_Encodable
        (Before_Literals, Before_Distances : Codebooks.Codebook;
         After_Literals, After_Distances   : Codebooks.Codebook) is
      begin
         Codebooks.Lemma_Ready_From_Fields (Before_Literals, After_Literals);
         Codebooks.Lemma_Ready_From_Fields (Before_Distances, After_Distances);
      ...
      with
        ...
        Pre  =>
          ...
          and then Before_Distances.Lengths = After_Distances.Lengths
          and then Before_Distances.Counts = After_Distances.Counts
          and then Payload.Covers (Before_Literals, Before_Distances, Data),
        Post => Payload.Covers (After_Literals, After_Distances, Data);
      ...
      with
        ...
        Pre  =>
          ...
          and then Books_Encodable (After_Literals, After_Distances)
          and then Payload.Covers (Before_Literals, Before_Distances, Data)
          and then Payload.Covers (After_Literals, After_Distances, Data)
          and then Is_Encoding
                     (Input,
                      ...
                      Data),
        Post =>
          Is_Encoding (Input, Produced, After_Literals, After_Distances, Data);
   ...
   begin
      ...
      Lemma_Header_Books_Recovered (Input, Literal_Lengths, Distances);
      pragma Assert (Literal_Book_From_Header (Input) = Literal_Lengths);
      pragma Assert (Distance_Book_From_Header (Input) = Distances);
   ...
   with
     ...
     Pre  =>
       ...
       and then (for all I in 0 .. Consumed - 1 =>
                   After (After'First + I) = Before (Before'First + I)),
     Post =>
       Fixed.Bit_Value (After, Position) = Fixed.Bit_Value (Before, Position);
   ...
   begin
      for Position in 0 .. Encoding_End - 1 loop
         pragma
           Loop_Invariant
             (for all P in 0 .. Position - 1 =>
                Fixed.Bit_Value (After, P) = Fixed.Bit_Value (Before, P));
         Lemma_Byte_Frame (Before, After, Produced, Position);
      end loop;

      Lemma_Header_Frame (Before, After, Literal_Lengths, Distances);
      Lemma_Header_Books_Recovered (After, Literal_Lengths, Distances);
      Payload.Lemma_Payload_Frame
        (Before, After, Header_Bit_Count, Literal_Lengths, Distances, Data);

      pragma
        Assert
          (Is_Encoding (After, Produced, Literal_Lengths, Distances, Data));
      Lemma_Encoding_Uses_Header_Books
   ...
   procedure Lemma_First_Separated
     (Book : Codebooks.Codebook; Short, Long : Codebooks.Code_Length_Pos)
   with
     Ghost,
     Pre                =>
       ...
       and then Short < Long,
     Post               =>
       Codebooks.First_Code (Book.Counts, Long) / Codebooks.Pow2 (Long - Short)
       >= Codebooks.First_Code (Book.Counts, Short) + Book.Counts (Short),
     Subprogram_Variant => (Decreases => Long - Short);

   procedure Lemma_First_Separated
     (Book : Codebooks.Codebook; Short, Long : Codebooks.Code_Length_Pos) is
   begin
      if Long > Short + 1 then
         ...
         declare
            ...
            A : constant Natural := Codebooks.First_Code (Book.Counts, Long);
   ...
   procedure Lemma_Double_Div (P, B : Natural; Q : Positive)
   with
     Ghost,
     Pre  =>
       P < Codebooks.Pow2 (15)
   ...
   procedure Lemma_Double_Div (P, B : Natural; Q : Positive) is null;
   ...
   procedure Lemma_Prefix_Shortens
     (Input       : Byte_Array;
      Start       : Natural;
      Short, Long : Codebooks.Code_Length_Pos) is
   begin
      if Long > Short + 1 then
         ...
         declare
            ...
            B : constant Natural := Fixed.Bit_Value (Input, Start + Long - 1);
            Q : constant Positive := Codebooks.Pow2 (Long - Short - 1);
         begin
            pragma Assert (P < Codebooks.Pow2 (Long - 1));
            pragma
              Assert (Fixed.Prefix_Value (Input, Start, Long) = 2 * P + B);
   ...
   with
     ...
     Post               =>
       Codebooks.Count_Length (Lengths, Length, Hi)
   ...
   procedure Lemma_Rank_Unique
     (Book : Codebooks.Codebook; A, B : Codebooks.Symbol_Index)
   with
     Ghost,
     Pre  =>
       Book.Kind = Codebooks.Canonical
       ...
       and then Codebooks.Length_Of (Book, B) = Codebooks.Length_Of (Book, A)
       and then Codebooks.Rank_Of (Book, A) = Codebooks.Rank_Of (Book, B),
     Post => A = B;
   ...
   with
     ...
     Pre  =>
       ...
       and then Start <= 8 * Input'Length
       and then Codebooks.Length_Of (Book, A) <= 8 * Input'Length - Start
       and then Codebooks.Length_Of (Book, B) <= 8 * Input'Length - Start
       and then Fixed.Prefix_Value
   ...
   procedure Lemma_Codewords_Unique
     (Input : Byte_Array;
      ...
      A, B  : Codebooks.Symbol_Index)
   is
      LA : constant Codebooks.Code_Length_Pos := Codebooks.Length_Of (Book, A);
      LB : constant Codebooks.Code_Length_Pos := Codebooks.Length_Of (Book, B);
   begin
      if LA < LB then
         ...
         pragma
           Assert (Codebooks.Code_Of (Book, A) = Codebooks.Code_Of (Book, B));
         pragma
           Assert (Codebooks.Rank_Of (Book, A) = Codebooks.Rank_Of (Book, B));
         ...
         begin
            ...
            declare
               Tail : constant Stream_Info :=
                 Spec_Walk
                   (Input,
                    Distance_Code.Position,
                    Literal_Lengths,
                    Distances,
                    Decoded + Length);
            begin
               if Tail.Valid then
                  return (True, Tail.End_Bit, Tail.Decoded_Length + Length);
               else
                  return (False, 0, 0);
               end if;
            end;
   ...
   with
     ...
     Post =>
       ...
       = (if Spec_Walk (Input, Position, Literal_Lengths, Distances, Decoded)
               .Valid
          then
            (True,
             Spec_Walk (Input, Position, Literal_Lengths, Distances, Decoded)
   ...
   begin
      Lemma_Walk_Start (Input, Start, Literal_Lengths, Distances, Decoded);
      loop
         pragma Loop_Invariant (P in Start .. 8 * Input'Length);
         pragma Loop_Invariant (Count <= Max_Input - Decoded);
         pragma
           Loop_Invariant
             (Spec_Walk (Input, Start, Literal_Lengths, Distances, Decoded)
                = (if Spec_Walk
   ...
   begin
      ...
      begin
         if not Books_Encodable (Literal_Lengths, Distances)
           or else not Header_Encodes (Input, Literal_Lengths, Distances)
         then
            return (False, 0, 0);
         end if;
         return Walk (Input, Header_Bit_Count, Literal_Lengths, Distances, 0);
      end;
   end Analyze;
      ...
      if not Literal_Code.Valid then
         ...
         begin
            if not Distance_Code.Valid then
               return (Fixed.Truncated, 0, 0, 0, Literal_Code.Position);
            elsif Distance_Code.Symbol <= 3 then
               return
                 (Fixed.Match,
                  0,
                  Literal_Code.Symbol - 254,
                  Distance_Code.Symbol + 1,
                  Distance_Code.Position);
            else
   ...
   begin
      if S.Position > End_Bit then
         ...
         case S.Kind is
            ...
            when Fixed.Match                                        =>
               return
                 Fixed.Match_Applies (Data, Index, S.Length, S.Distance)
                 and then Spec_Matches
                            (Input,
                             End_Bit,
                             Literal_Lengths,
   ...
   begin
      while Index < Data'Length loop
         ...
         S := Next_Symbol (Input, Position, Literal_Lengths, Distances);
         if S.Position > End_Bit then
            return False;
         end if;
         case S.Kind is
            ...
            when Fixed.Match                                        =>
               if not Fixed.Match_Applies (Data, Index, S.Length, S.Distance)
               then
                  return False;
               end if;
               Position := S.Position;
   ...
   with
     ...
     Post =>
       Decode_Symbol (Input, Start, Book).Valid
       and then Decode_Symbol (Input, Start, Book).Symbol = Expected
       and then Decode_Symbol (Input, Start, Book).Position
   ...
   begin
      ...
      Lemma_Codewords_Unique (Input, Start, Book, Result.Symbol, Expected);
   ...
   with
     ...
     Post               =>
       Spec_Walk
         (Input,
          Start + Payload.Data_Bits (Literal_Lengths, Distances, Data, Index),
          Literal_Lengths,
          Distances,
          Index)
         .Valid
   ...
   begin
      if Index = Data'Length then
         pragma
           Assert (Code_Matches (Input, Bit_Position, Literal_Lengths, 256));
         Lemma_Codeword_Decodes (Input, Bit_Position, Literal_Lengths, 256);
         pragma
           Assert
             (Next_Symbol (Input, Bit_Position, Literal_Lengths, Distances)
                .Kind
                = Fixed.End_Of_Block);
         pragma
           Assert
         ...
         begin
            ...
            pragma
              Assert
                (Code_Matches (Input, Bit_Position, Literal_Lengths, Symbol));
                    ...
                    Assert
                      (Code_Matches
                         (Input,
                          ...
                          Distance_Symbol));
                  Lemma_Codeword_Decodes
                    (Input, Literal_Code.Position, Distances, Distance_Symbol);
                  pragma
                    Assert
                      (Next_Symbol
                         (Input, Bit_Position, Literal_Lengths, Distances)
                         .Kind
                         = Fixed.Match);
                  pragma
                    Assert
   ...
   begin
      ...
      Lemma_Payload_Analyzes
        (Input, Header_Bit_Count, Literal_Lengths, Distances, Data, 0);
      pragma
        Assert
          (Walk (Input, Header_Bit_Count, Literal_Lengths, Distances, 0)
             .Valid);
   ...
   with
     ...
     Pre  =>
       ...
       and then Left (Left'First .. Left'First + Count - 1)
   ...
   with
     ...
     Pre  =>
       ...
       and then Left (Left'First + Index .. Left'First + Index + Count - 1)
                = Right
                    (Right'First + Index .. Right'First + Index + Count - 1)
       and then Left (Left'First + Index + Count)
   ...
   with
     ...
     Pre  =>
       ...
       and then Fixed.Match_Applies (Left, Index, Length, Distance)
       and then Fixed.Match_Applies (Right, Index, Length, Distance)
       and then Left (Left'First .. Left'First + Index - 1)
                = Right (Right'First .. Right'First + Index - 1),
     Post =>
       Left (Left'First + Index .. Left'First + Index + Length - 1)
       = Right (Right'First + Index .. Right'First + Index + Length - 1);

   procedure Lemma_Match_Functional
     (Left, Right : Byte_Array; Index, Length, Distance : Natural) is
   begin
      for K in 0 .. Length - 1 loop
         ...
         pragma
           Loop_Invariant
             (Left (Left'First + Index .. Left'First + Index + K - 1)
                = Right (Right'First + Index .. Right'First + Index + K - 1));
         if K < Distance then
         ...
         pragma
           Assert
             (Left (Left'First + Index + K) = Right (Right'First + Index + K));
         Lemma_Slice_Extend_One (Left, Right, Index, K);
      end loop;
   end Lemma_Match_Functional;

   procedure Lemma_Slice_To_Prefix (Left, Right : Byte_Array; Count : Natural)
   with
     Ghost,
     Pre  =>
       Count <= Left'Length
   ...
   with
     ...
     Pre  =>
       ...
       and then Left (Left'First .. Left'First + Index - 1)
                = Right (Right'First .. Right'First + Index - 1)
       and then Left (Left'First + Index .. Left'First + Index + Length - 1)
                = Right
   ...
   with
     ...
     Pre                =>
       ...
       and then Right'Length <= Fixed.Max_Input
       and then Payload.Covers (Literal_Lengths, Distances, Left)
       and then Payload.Covers (Literal_Lengths, Distances, Right)
       and then Payload.Is_Encoding
       ...
       and then Payload.Data_Bits (Literal_Lengths, Distances, Left, Index)
                = Payload.Data_Bits (Literal_Lengths, Distances, Right, Index)
       and then Left (Left'First .. Left'First + Index - 1)
                = Right (Right'First .. Right'First + Index - 1),
     Post               =>
   ...
   is
      Bit_Position : constant Natural :=
        Start + Payload.Data_Bits (Literal_Lengths, Distances, Left, Index);
   begin
            ...
            if Left_Token.Kind = Fixed.Match then
               ...
               declare
                  Literal_Length : constant Positive :=
                    Codebooks.Length_Of (Literal_Lengths, Left_Symbol);
               ...
               begin
                  Lemma_Codewords_Unique
                    (Input,
                     Bit_Position + Literal_Length,
                     Distances,
                     Left_Distance,
                     Right_Distance);
                  pragma Assert (Left_Token.Distance = Right_Token.Distance);
   ...
   begin
      ...
      pragma
        Assert (Payload.Data_Bits (Literal_Lengths, Distances, Left, 0) = 0);
      pragma
        Assert (Payload.Data_Bits (Literal_Lengths, Distances, Right, 0) = 0);
      Lemma_Payload_Functional
        (Input, Header_Bit_Count, Literal_Lengths, Distances, Left, Right, 0);
   ...
   begin
      if Index = Left'Length then
         ...
         pragma
           Assert (Left (Left'First + Index) = Right (Right'First + Index));
         Lemma_Slice_Extend_One (Left, Right, 0, Index);
         ...
         Lemma_Match_Functional (Left, Right, Index, S.Length, S.Distance);
          ...
          = (Next_Symbol (Input, Position, Literal_Lengths, Distances).Position
             ...
             and then Decoded_Prefix_Matches
                        (Input,
                         ...
                         End_Index)),
        others                                            =>
          not Decoded_Prefix_Matches'Result),
   ...
   begin
      if Position = End_Position or else Index = End_Index then
         ...
         return
           ...
           and then Fixed.Match_Applies (Data, Index, S.Length, S.Distance)
           and then Decoded_Prefix_Matches
                      (Input,
                       Literal_Lengths,
                       Distances,
   ...
   procedure Lemma_Decoded_Match_Frame
     (Before, After : Byte_Array; Count, Index, Length, Distance : Natural)
   with
     Ghost,
     Pre  =>
       ...
       and then Fixed.Match_Applies (Before, Index, Length, Distance)
       and then (for all I in 0 .. Count - 1 =>
                   Before (Before'First + I) = After (After'First + I)),
     Post => Fixed.Match_Applies (After, Index, Length, Distance);

   procedure Lemma_Decoded_Match_Frame
     (Before, After : Byte_Array; Count, Index, Length, Distance : Natural) is
   begin
      for K in 0 .. Length - 1 loop
         pragma
           Loop_Invariant
         ...
         Lemma_Decoded_Prefix_Element (Before, After, Count, Index + K);
         pragma
           Assert
     ...
     Pre                =>
       ...
       and then Decoded_Prefix_Matches
                  (Input,
                   ...
                   End_Position,
                   End_Index)
       and then (for all I in 0 .. End_Index - 1 =>
                   Before (Before'First + I) = After (After'First + I)),
     Post               =>
       Decoded_Prefix_Matches
         (Input,
   ...
   begin
      if Position = End_Position then
         ...
         Lemma_Decoded_Prefix_Element (Before, After, End_Index, Index);
   ...
   procedure Lemma_Decoded_Prefix_Snoc
     (Input                                                             :
      ...
      Data                                                              :
        Byte_Array;
      Position, Index, Old_Position, Old_Index, New_Position, New_Index :
        Natural)
   with
     Ghost,
     Pre                =>
     ...
     Pre  =>
       ...
       and then Decoded_One_Token
                  (Input,
                   ...
                   New_Position,
                   New_Index)
       and then (for all I in 0 .. Old_Index - 1 =>
                   Before (Before'First + I) = After (After'First + I)),
     Post =>
       Decoded_Prefix_Matches
         (Input,
   ...
   with
     ...
     Pre                =>
       ...
       and then Decoded_Prefix_Matches
                  (Input,
                   ...
                   End_Position,
                   End_Index)
       and then Next_Symbol (Input, End_Position, Literal_Lengths, Distances)
                  .Kind
                = Fixed.End_Of_Block
       and then Next_Symbol (Input, End_Position, Literal_Lengths, Distances)
                  .Position
     ...
     Post               =>
       Spec_Matches
         (Input, End_Bit, Literal_Lengths, Distances, Data, Position, Index),
     Subprogram_Variant => (Decreases => End_Position - Position);
   ...
   with
     ...
     Post =>
       ...
       and then (for all I in 0 .. Produced'Old - 1 =>
                   Data (Data'First + I) = Data'Old (Data'First + I))
       and then Fixed.Match_Applies (Data, Produced'Old, Length, Distance);
   ...
   is
      ...
      Before : constant Byte_Array := Data
      with Ghost;
   begin
      for K in 0 .. Length - 1 loop
         pragma
         ...
         pragma
           Loop_Invariant
             (for all J in 0 .. K - 1 =>
                Data (First + P + J) = Data (First + P + J - Distance));
         Data (First + P + K) := Data (First + P + K - Distance);
      end loop;
      Produced := P + Length;
   end Copy_Decoded_Match;

   procedure Lemma_Decoded_Bit_Prefix_Frame
     (Before, After : Byte_Array; Consumed, Start, Length : Natural)
   with
     Ghost,
     Pre                =>
       Before'Length <= Fixed.Max_Stream_Bytes
       ...
       and then (for all I in 0 .. Consumed - 1 =>
                   After (After'First + I) = Before (Before'First + I)),
     Post               =>
       Fixed.Prefix_Value (After, Start, Length)
       = Fixed.Prefix_Value (Before, Start, Length),
     Subprogram_Variant => (Decreases => Length);

   procedure Lemma_Decoded_Bit_Prefix_Frame
     (Before, After : Byte_Array; Consumed, Start, Length : Natural) is
   begin
      if Length > 0 then
         Lemma_Decoded_Bit_Prefix_Frame
           (Before, After, Consumed, Start, Length - 1);
         Lemma_Byte_Frame (Before, After, Consumed, Start + Length - 1);
      end if;
   ...
   with
     ...
     Pre  =>
       ...
       and then Books_Encodable (Literal_Lengths, Distances)
       and then Position <= 8 * Consumed
       and then Next_Symbol (Before, Position, Literal_Lengths, Distances).Kind
                in Fixed.Literal | Fixed.Match | Fixed.End_Of_Block
       and then Next_Symbol (Before, Position, Literal_Lengths, Distances)
                  .Position
                <= 8 * Consumed
       and then (for all I in 0 .. Consumed - 1 =>
                   After (After'First + I) = Before (Before'First + I)),
     Post =>
       Next_Symbol (After, Position, Literal_Lengths, Distances)
   ...
   is
      S              : constant Fixed.Symbol_Result :=
        Next_Symbol (Before, Position, Literal_Lengths, Distances);
      Symbol         : constant Codebooks.Symbol_Index :=
        (if S.Kind = Fixed.Literal
         then Natural (S.Value)
         elsif S.Kind = Fixed.Match
         then S.Length + 254
         else 256);
      Literal_Length : constant Positive :=
   ...
   begin
      ...
      Lemma_Codeword_Decodes (After, Position, Literal_Lengths, Symbol);
      if S.Kind = Fixed.Match then
         ...
         Lemma_Codeword_Decodes
           (After, Position + Literal_Length, Distances, S.Distance - 1);
      end if;
      pragma
        Assert (Next_Symbol (After, Position, Literal_Lengths, Distances) = S);
   ...
   with
     ...
     Post               =>
       Spec_Matches
         (After, End_Bit, Literal_Lengths, Distances, Data, Position, Index),
     Subprogram_Variant =>
   ...
   begin
      Lemma_Decoded_Next_Frame
        (Before, After, Consumed, Position, Literal_Lengths, Distances);
      if Index < Data'Length then
         if S.Kind = Fixed.Literal then
     ...
     Pre                =>
       ...
       and then Spec_Matches
                  (Input,
                   ...
                   Index),
     Post               =>
       Spec_Walk (Input, Position, Literal_Lengths, Distances, Index).Valid
       and then Spec_Walk (Input, Position, Literal_Lengths, Distances, Index)
                  .End_Bit
                = End_Bit
       and then Spec_Walk (Input, Position, Literal_Lengths, Distances, Index)
                  .Decoded_Length
   ...
   begin
      ...
      if Left.Valid then
         Codebooks.Lemma_Code_Of_From_Fields (Before, After, Left.Symbol);
         pragma Assert (Code_Matches (Input, Position, After, Left.Symbol));
         ...
         Codebooks.Lemma_Code_Of_From_Fields (Before, After, Right.Symbol);
         pragma Assert (Code_Matches (Input, Position, Before, Right.Symbol));
         pragma Assert (False);
      end if;
   ...
   with
     ...
     Pre  =>
       ...
       and then Books_Encodable (After_Literals, After_Distances),
     Post =>
       Next_Symbol (Input, Position, Before_Literals, Before_Distances)
       = Next_Symbol (Input, Position, After_Literals, After_Distances);
   ...
   begin
      Lemma_Equal_Books_Encodable
        (Before_Literals, Before_Distances, After_Literals, After_Distances);
      Lemma_Equal_Book_Decode
        (Input, Position, Before_Literals, After_Literals);
      if Literal_Code.Valid and then Literal_Code.Symbol in 257 .. 264 then
         Lemma_Equal_Book_Decode
           (Input, Literal_Code.Position, Before_Distances, After_Distances);
      end if;
   ...
   with
     ...
     Pre                =>
       ...
       and then Books_Encodable (After_Literals, After_Distances),
     Post               =>
       Spec_Walk (Input, Position, Before_Literals, Before_Distances, Decoded)
       = Spec_Walk (Input, Position, After_Literals, After_Distances, Decoded),
      ...
      if not Literal_Code.Valid then
         ...
         declare
            Length              : constant Natural :=
              Literal_Code.Symbol - 254;
            Distance_Code       : constant Decoded_Symbol :=
              Decode_Symbol (Input, Literal_Code.Position, Before_Distances);
            After_Distance_Code : constant Decoded_Symbol :=
              Decode_Symbol (Input, Literal_Code.Position, After_Distances);
         begin
            Lemma_Equal_Book_Decode
              (Input,
               Literal_Code.Position,
   ...
   with
     ...
     Pre  =>
       ...
       and then Books_Encodable (After_Literals, After_Distances),
     Post =>
       Walk (Input, Position, Before_Literals, Before_Distances, Decoded)
       = Walk (Input, Position, After_Literals, After_Distances, Decoded);
   ...
   begin
      ...
      pragma Assert (Books_Encodable (Literal_Lengths, Distances));
      Lemma_Header_Frame (Before, After, Literal_Lengths, Distances);
      pragma Assert (Header_Encodes (After, Literal_Lengths, Distances));
      Lemma_Header_Books_Recovered (After, Literal_Lengths, Distances);
      pragma Assert (Literal_Book_From_Header (After) = Literal_Lengths);
      pragma Assert (Distance_Book_From_Header (After) = Distances);
      Lemma_Equal_Books_Encodable
        (Literal_Lengths,
         Distances,
      ...
      pragma
        Assert
          (Walk (After, Header_Bit_Count, Literal_Lengths, Distances, 0)
             .Valid);
      pragma
        Assert
          (Walk (After, Header_Bit_Count, Literal_Lengths, Distances, 0)
      ...
      pragma Assert (Analyze (After).Decoded_Length = Data'Length);
   ...
   begin
      ...
      begin
         loop
            pragma
              Loop_Invariant (Position in Header_Bit_Count .. Info.End_Bit);
            pragma Loop_Invariant (Count <= Info.Decoded_Length);
            pragma
              Loop_Invariant
                (Spec_Walk (Input, Position, Literal_Lengths, Distances, Count)
                   .Valid);
            pragma
              Loop_Invariant
                (Spec_Walk (Input, Position, Literal_Lengths, Distances, Count)
            ...
            pragma
              Loop_Invariant
                (Count
                   + Spec_Walk
                       (Input, Position, Literal_Lengths, Distances, Count)
                       .Decoded_Length
                   = Info.Decoded_Length);
            pragma
              Loop_Invariant
            ...
            case S.Kind is
                     ...
                     pragma
                       Assert
                         (for all I in 0 .. Old_Count - 1 =>
                            Data (Data'First + I) = Before (Before'First + I));
                     pragma
                       Assert
                         (Decoded_One_Token
                            (Input,
               ...
               when Fixed.Match                   =>
                  ...
                  pragma Assert (S.Length <= Info.Decoded_Length - Count);
                  declare
                     Before       : constant Byte_Array := Data
                     with Ghost;
                     Old_Position : constant Natural := Position;
                     Old_Count    : constant Natural := Count;
                  begin
                     Copy_Decoded_Match (Data, Count, S.Length, S.Distance);
                     Position := S.Position;
                     pragma
                       Assert
                         (Decoded_One_Token
