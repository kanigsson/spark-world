...
is
   ...
   with
     ...
     Pre                =>
       ...
       and then DL = DF - 1
       and then Model.Encodes_Stored (Input, CF, CL, Data, DF, DL),
     Post               => Model.Encodes_Stored (Input, CF, CL, Data, 1, 0),
   ...
   begin
      pragma Assert (Len = 0);
      if Input (CF) = 0 then
         Lemma_Empty_Reindex (Input, CF + 5 + Len, CL, Data, DF + Len, DL);
      end if;
   ...
   procedure Lemma_Encoding_Recognized
     (Input : Byte_Array; Consumed : Natural; Data : Byte_Array)
   is
      DF : constant Positive := (if Data'Length > 0 then Data'First else 1);
      DL : constant Natural := (if Data'Length > 0 then Data'Last else 0);
   begin
      if Input'Length >= 5
   ...
   is
      LF : constant Positive := (if Left'Length > 0 then Left'First else 1);
      LL : constant Natural := (if Left'Length > 0 then Left'Last else 0);
      RF : constant Positive := (if Right'Length > 0 then Right'First else 1);
      RL : constant Natural := (if Right'Length > 0 then Right'Last else 0);
   begin
      if Input'Length >= 5
        and then Model.Encodes_Stored
                   (Input,
      ...
      then
         ...
         Fixed.Lemma_Encoding_Functional (Input, Consumed, Left, Right);
      else
         ...
         Dynamic.Lemma_Decoding_Functional (Input, Consumed, Left, Right);
      end if;
