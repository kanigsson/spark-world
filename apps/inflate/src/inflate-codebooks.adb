package body Inflate.Codebooks
  with SPARK_Mode => On
is
   ...
   with
     ...
     Post               =>
       Count_Length (Lengths, Length, Last)
   ...
   function Rank_Of
     (Book : Codebook; Symbol : Symbol_Index) return Symbol_Count is
   begin
      Lemma_Count_Step
         ...
         pragma
           Loop_Invariant
             (for all Length in Code_Length_Pos =>
                Book.Counts (Length) = Count_Length (Lengths, Length, Symbol));
   ...
   procedure Lemma_Exact_Books_Equal (Left, Right : Codebook) is
      ...
      with
        ...
        Post               =>
          Count_Length (Left_Lengths, Length, Count)
   ...
   procedure Lemma_Canonical_Lengths_Equal (Left, Right : Codebook) is
   begin
      pragma
        Assert
   ...
   with
     ...
     Pre                => Left = Right,
     Post               =>
       Count_Length (Left, Length, Count)
   ...
   procedure Lemma_First_Codes_Equal
     (Left, Right : Length_Count_Array; Length : Code_Length_Pos)
   with
     Ghost,
     Pre                => Left = Right,
     Post               =>
       First_Code (Left, Length) = First_Code (Right, Length),
     Subprogram_Variant => (Decreases => Length);

   procedure Lemma_First_Codes_Equal
     (Left, Right : Length_Count_Array; Length : Code_Length_Pos) is
   begin
      if Length > 1 then
   ...
   procedure Lemma_Kraft_Values_Equal
     (Left, Right : Length_Count_Array; Length : Code_Length)
   with
     Ghost,
     Pre                => Left = Right,
     Post               =>
       Kraft_Value (Left, Length) = Kraft_Value (Right, Length),
     Subprogram_Variant => (Decreases => Length);

   procedure Lemma_Kraft_Values_Equal
     (Left, Right : Length_Count_Array; Length : Code_Length) is
   begin
      if Length > 0 then
   ...
   procedure Lemma_Ready_From_Fields (Left, Right : Codebook) is
   begin
      ...
      for Length in Code_Length_Pos loop
         ...
         pragma
           Loop_Invariant
             (for all L in Code_Length_Pos'First .. Length - 1 =>
                First_Code (Right.Counts, L) + Right.Counts (L) <= Pow2 (L));
   ...
   procedure Lemma_Lengths_At_Most_From_Fields
     (Left, Right : Codebook; Maximum : Code_Length) is
   begin
      for Symbol in Symbol_Index loop
         pragma
           Loop_Invariant
   ...
   procedure Lemma_Length_Of_From_Fields (Left, Right : Codebook) is
   begin
      for Symbol in Symbol_Index loop
         pragma
           Loop_Invariant
         ...
         pragma Assert (Left.Lengths (Symbol) = Right.Lengths (Symbol));
         pragma Assert (Length_Of (Left, Symbol) = Length_Of (Right, Symbol));
      end loop;
   ...
   begin
      ...
      Lemma_Count_Lengths_Equal (Left.Lengths, Right.Lengths, Length, Symbol);
      pragma
        Assert
          (Rank_Of (Left, Symbol)
