...
is
   ...
   is (A => Component ((Adler and 16#FFFF#) mod Base),
   ...
   function Model_Byte_Step (S : State; Value : Byte) return State is
      Next_A : constant Component := S.A + Component (Value);
   begin
      return (A => Next_A, B => S.B + Next_A);
   end Model_Byte_Step;
   ...
   is (Shift_Left (Word32 (S.B), 16) or Word32 (S.A));
   ...
   function Fold
     (S : State; Data : Byte_Array; From : Positive; To : Natural) return State
   is (if From > To
       then S
       else Fold (Model_Byte_Step (S, Data (From)), Data, From + 1, To));
