...
is
   ...
   procedure Lemma_Prefix_Frame
     (Before, After : Byte_Array; Start, Length : Natural)
   with
     Ghost,
     Pre                =>
       ...
       and then (for all Position in Start .. Start + Length - 1 =>
   ...
   procedure Lemma_Prefix_Frame
     (Before, After : Byte_Array; Start, Length : Natural) is
   begin
      if Length > 0 then
   ...
   with
     ...
     Post =>
       Fixed.Prefix_Value (Output, Start, Length) = Value
     ...
     Post =>
       ...
       and then Data_Bits
                ...
                = Data_Bits (Literal_Lengths, Distances, Data, Position)
                  + Token_Bit_Cost (Literal_Lengths, Distances, Data, Position)
       and then (for all Count in
                   Position + 1 .. Fixed.Next_Position (Data, Position) - 1 =>
                   not Fixed.Token_Boundary (Data, Count)
                   and then Data_Bits (Literal_Lengths, Distances, Data, Count)
                            = Data_Bits
   ...
   begin
      ...
      for Count in Position + 1 .. Next - 1 loop
         pragma
           Loop_Invariant
             (Data_Bits (Literal_Lengths, Distances, Data, Count - 1)
                = Data_Bits (Literal_Lengths, Distances, Data, Position));
   ...
   procedure Lemma_Data_Bits_Advance
     (Literal_Lengths : Codebooks.Codebook;
      ...
      Position        : Natural) is
   begin
      Lemma_Data_Bits_Next (Literal_Lengths, Distances, Data, Position);
   ...
   with
     ...
     Pre                =>
       ...
       and then Codebooks.Lengths_At_Most (Distances, 9),
     Post               =>
       Fixed.Next_Position (Data, Position) <= Count
       and then Data_Bits (Literal_Lengths, Distances, Data, Position)
                + Token_Bit_Cost (Literal_Lengths, Distances, Data, Position)
                <= Data_Bits (Literal_Lengths, Distances, Data, Count),
   ...
   procedure Lemma_Last_Boundary (Data : Byte_Array) is
      Index : Natural := 0;
   begin
   ...
   with
     ...
     Pre  =>
       ...
       and then Start + Data_Bits (Literal_Lengths, Distances, Data, Count)
                <= 8 * After'Length
       and then (for all Position in
                   ...
                   = Fixed.Bit_Value (Before, Position)),
     Post =>
       Encodes_Prefix (After, Start, Literal_Lengths, Distances, Data, Count);
         ...
         if Fixed.Token_Boundary (Data, I) then
            ...
            declare
               ...
               Position : constant Natural :=
                 Start + Data_Bits (Literal_Lengths, Distances, Data, I);
            begin
   ...
   is
      Payload_End : constant Natural :=
        Start + Data_Bits (Literal_Lengths, Distances, Data, Data'Length);
   begin
      Lemma_Last_Boundary (Data);
      Lemma_Encodes_Frame
        (Before, After, Start, Literal_Lengths, Distances, Data, Data'Length);
      Lemma_Prefix_Frame
        (Before,
         After,
         Payload_End,
   ...
   procedure Lemma_Encodes_Add
     (Output           : Byte_Array;
      ...
      Distances        : Codebooks.Codebook;
      Data             : Byte_Array;
      Index, New_Index : Natural) is
   begin
      Lemma_Data_Bits_Next (Literal_Lengths, Distances, Data, Index);
      ...
      pragma
        Assert
          (Fits_At
             (Output,
              Start,
              Data_Bits (Literal_Lengths, Distances, Data, New_Index),
              0));
   ...
   begin
      ...
      while Index < Data'Length loop
         pragma Loop_Invariant (Index <= Data'Length);
         pragma Loop_Invariant (Fixed.Token_Boundary (Data, Index));
         pragma Loop_Invariant (Fixed.Token_Boundary (Data, Data'Length));
         pragma
           Loop_Invariant
             (Bits
                = Start + Data_Bits (Literal_Lengths, Distances, Data, Index));
         pragma
           Loop_Invariant
             (Fits_At
                (Output,
         ...
         begin
            ...
            pragma
              Assert
                (Fits_At
                   (Output,
                    Start,
                    Data_Bits (Literal_Lengths, Distances, Data, Old_Index),
                    Token_Bit_Cost
            ...
            if Token.Kind = Fixed.Match then
               ...
               begin
                  ...
                  declare
                     Before_Distance : constant Byte_Array := Output
                     with Ghost;
                  begin
                     Write_Code
                       (Output,
            ...
            pragma
              Assert
                (Bits
                   = Start
                     + Data_Bits (Literal_Lengths, Distances, Data, Index));
            pragma
              Assert
                (for all Position in 0 .. 8 * Output'Length - 1 =>
                   (if Position < Start or else Position >= Bits
      ...
      declare
         Before_End : constant Byte_Array := Output
         with Ghost;
      begin
         Write_Code
           (Output,
   ...
   with
     ...
     Post =>
       Token_Bit_Cost (Before_Literals, Before_Distances, Data, Position)
       = Token_Bit_Cost (After_Literals, After_Distances, Data, Position);
   ...
   with
     ...
     Post =>
       Token_Encoded
         (Output, Start, After_Literals, After_Distances, Data, Position);
   ...
   begin
      ...
      if Token.Kind = Fixed.Match then
         Codebooks.Lemma_Code_Of_From_Fields
           (Before_Literals, After_Literals, Length_Symbol (Token.Length));
         Codebooks.Lemma_Code_Of_From_Fields
           (Before_Distances,
            After_Distances,
            Distance_Symbol (Token.Distance));
         ...
         Codebooks.Lemma_Code_Of_From_Fields
           (Before_Literals, After_Literals, Natural (Token.Value));
      end if;
      pragma
        Assert
          (Token_Encoded
   ...
   procedure Lemma_Covers_From_Lengths
     (Before_Literals, Before_Distances : Codebooks.Codebook;
      ...
      Data                              : Byte_Array) is
   begin
      pragma Assert (Codebooks.Length_Of (After_Literals, 256) > 0);
                 ...
                 then
                   (if Fixed.Selected_Token (Data, J).Kind = Fixed.Match
                    then
                      Codebooks.Length_Of
                        (After_Literals,
                         Length_Symbol (Fixed.Selected_Token (Data, J).Length))
                      > 0
                      ...
                      Codebooks.Length_Of
                        (After_Literals, Natural (Data (Data'First + J)))
                      > 0)));
         if Fixed.Token_Boundary (Data, I) then
            if Fixed.Selected_Token (Data, I).Kind = Fixed.Match then
               pragma
                 Assert
                   (Codebooks.Length_Of
                      (Before_Literals,
                       Length_Symbol (Fixed.Selected_Token (Data, I).Length))
                      > 0);
               pragma
                 Assert
                   (Codebooks.Length_Of
                      (Before_Distances,
                       Distance_Symbol
                         (Fixed.Selected_Token (Data, I).Distance))
                      > 0);
            else
               pragma
                 Assert
                   (Codebooks.Length_Of
                      (Before_Literals, Natural (Data (Data'First + I)))
                      > 0);
            end if;
         end if;
      end loop;
