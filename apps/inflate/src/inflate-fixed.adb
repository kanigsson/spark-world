...
is
   ...
   with
     ...
     Post =>
       Matching_Length'Result <= 10
       and then Matching_Length'Result <= Data'Length - Position
       and then (for all K in 0 .. Matching_Length'Result - 1 =>
   ...
   begin
      ...
      if Best_Length >= 3 then
         return (Match, 0, Best_Length, Best_Distance, Position + Best_Length);
      else
         return (Literal, Data (Data'First + Position), 1, 0, Position + 1);
      end if;
   end Selected_Token;
   ...
   function Prefix_Value
     (Input : Byte_Array; Start : Natural; Length : Natural) return Natural is
   begin
      ...
      begin
         ...
         Bit_Cursors.Lemma_Field_Value_Recursion
           (Input,
            Start,
            Length,
            Bit_Cursors.Lsb_First,
            Bit_Cursors.High_Bit_First);
   ...
   begin
      case S.Kind is
         ...
         when Match             =>
            if S.Distance > Decoded or else S.Length > Max_Input - Decoded then
               return (False, 0, 0);
            else
               declare
                  Tail : constant Stream_Info :=
                ...
                = (if Spec_Walk (Input, P, Decoded + Count).Valid
                   then
                     (True,
                      ...
                      Count
                      + Spec_Walk (Input, P, Decoded + Count).Decoded_Length)
                   else (False, 0, 0)));
   ...
   with
     Ghost,
     Pre  => Prefix <= 511 and then Bit <= 1 and then Whole = 2 * Prefix + Bit,
     Post => Prefix = Whole / 2;
   ...
   with
     ...
     Pre  =>
       ...
       and then Prefix_Value (Input, Position, Code_Length (B)) = Code (B),
     Post =>
   ...
   procedure Lemma_Code_Decodes
     (Input : Byte_Array; Position : Natural; B : Byte) is
   begin
      if B <= 143 then
         ...
         pragma Assert (Prefix_Value (Input, Position, 7) = Code (B) / 2);
         ...
         pragma Assert (Prefix_Value (Input, Position, 8) = Code (B) / 2);
         Lemma_Prefix_Step (Input, Position, 8);
         Lemma_Half
           (Prefix_Value (Input, Position, 8),
            Prefix_Value (Input, Position, 7),
   ...
   procedure Lemma_EOB_Decodes (Input : Byte_Array; Position : Natural)
   with
     Ghost,
     Pre  =>
       Input'Length <= Max_Stream_Bytes
   ...
   procedure Lemma_EOB_Decodes (Input : Byte_Array; Position : Natural) is
   begin
      pragma Assert (Prefix_Value (Input, Position, 7) = 0);
   ...
   with
     ...
     Pre                =>
       ...
       and then Spec_Matches (Input, Consumed, Data, Position, Index),
     Post               =>
       Walk (Input, Position, Index).Valid
       and then Walk (Input, Position, Index).End_Bit <= 8 * Consumed
       and then (Walk (Input, Position, Index).End_Bit + 7) / 8 = Consumed
   ...
   begin
      ...
      if Index = Data'Length then
         ...
         Lemma_Matches_Walk (Input, Consumed, Data, S.Position, Index + 1);
      else
   ...
   procedure Lemma_Encoding_Analyzes
     (Input : Byte_Array; Consumed : Natural; Data : Byte_Array) is
   begin
   ...
   with
     Pre  =>
       Output'Length <= Max_Stream_Bytes
       and then Position < 8 * Output'Length
       and then Value <= 1,
     Post =>
       Bit_Value (Output, Position) = Value
   ...
   with
     Ghost,
     Pre  =>
       Before'Length <= Max_Stream_Bytes
   ...
   begin
      if Length > 0 then
         Lemma_Byte_Prefix_Frame (Before, After, Consumed, Start, Length - 1);
   ...
   with
     ...
     Post               =>
       Next_Position (Data, Position) <= Count
       and then Data_Bits (Data, Position) + Token_Bit_Cost (Data, Position)
   ...
   begin
      if S.Position > 8 * Consumed then
         ...
         return S.Kind = End_Of_Block and then Consumed = (S.Position + 7) / 8;
      else
         case S.Kind is
            when Literal                          =>
               return
            ...
            when Match                            =>
               return
                 Match_Applies (Data, Index, S.Length, S.Distance)
                 and then Spec_Matches
                            (Input,
                             Consumed,
                             Data,
   ...
   function Encoding_Matches
     (Input : Byte_Array; Consumed : Natural; Data : Byte_Array) return Boolean
   is
      Position : Natural := 3;
      Index    : Natural := 0;
      S        : Symbol_Result;
   begin
      while Index < Data'Length loop
         ...
         case S.Kind is
            ...
            when Match                            =>
               if not Match_Applies (Data, Index, S.Length, S.Distance) then
                  return False;
               end if;
               Position := S.Position;
   ...
   with
     ...
     Pre  =>
       ...
       and then 3 + Data_Bits (Data, Data'Length) + 7 <= 8 * Input'Length
       and then Encodes_Prefix (Input, Data, Data'Length)
       and then Prefix_Value (Input, 3 + Data_Bits (Data, Data'Length), 7) = 0
       and then Consumed = (3 + Data_Bits (Data, Data'Length) + 7 + 7) / 8,
     Post => Encoding_Matches (Input, Consumed, Data);

   procedure Lemma_Encoding_Next (Data : Byte_Array; Index : Natural) is
   ...
   begin
      for Offset in 1 .. S.Length loop
            ...
            pragma
              Assert
                (Plan_Remaining (Data, Index + Offset - 1)
                   = S.Length - Offset + 1);
            pragma Assert (Plan_Remaining (Data, Index + Offset - 1) > 0);
         end if;
         pragma
           Assert (Plan_Remaining (Data, Index + Offset) = S.Length - Offset);
         pragma Assert (Plan_Start (Data, Index + Offset) = Index);
         if Offset < S.Length then
            pragma Assert (not Token_Boundary (Data, Index + Offset));
            pragma
              Assert
                (Data_Bits (Data, Index + Offset) = Data_Bits (Data, Index));
         else
            pragma Assert (Token_Boundary (Data, Index + Offset));
            pragma
              Assert
         ...
         pragma Loop_Invariant (Plan_Start (Data, Index + Offset) = Index);
         pragma
           Loop_Invariant
             (if Offset < S.Length
      ...
      pragma Assert (Plan_Start (Data, Next_Position (Data, Index)) = Index);
   ...
   begin
      if Index = Data'Length then
         ...
         pragma Assert (Spec_Matches (Input, Consumed, Data, Position, Index));
      else
         S := Selected_Token (Data, Index);
         Lemma_Encoding_Next (Data, Index);
         Lemma_Data_Bits_Segment (Data, Index, Data'Length);
         if S.Kind = Match then
            Lemma_Match_Decodes (Input, Position, S.Length, S.Distance);
            Lemma_Compressor_Spec (Input, Consumed, Data, S.Position);
            pragma
              Assert
                (Spec_Matches
                   (Input, Consumed, Data, Position + 12, S.Position));
         else
            Lemma_Code_Decodes (Input, Position, S.Value);
            Lemma_Compressor_Spec (Input, Consumed, Data, S.Position);
            pragma
              Assert
                (Spec_Matches
   ...
   with
     ...
     Pre  =>
       ...
       and then Consumed <= After'Length
       and then Position <= 8 * Consumed
       and then Next_Symbol (Before, Position).Kind
                in Literal | Match | End_Of_Block
       and then Next_Symbol (Before, Position).Position <= 8 * Consumed
       and then (for all I in 0 .. Consumed - 1 =>
                   After (After'First + I) = Before (Before'First + I)),
     Post => Next_Symbol (After, Position) = Next_Symbol (Before, Position);
   ...
   begin
      case S.Kind is
         ...
         when Match             =>
            Lemma_Byte_Prefix_Frame (Before, After, Consumed, Position, 7);
            Lemma_Byte_Prefix_Frame (Before, After, Consumed, Position + 7, 5);
            Lemma_Match_Decodes (After, Position, S.Length, S.Distance);
            pragma Assert (Next_Symbol (After, Position) = S);
   ...
   with
     ...
     Pre                =>
       ...
       and then Position <= 8 * Consumed
       and then Index <= Data'Length
       and then Spec_Matches (Before, Consumed, Data, Position, Index)
       and then (for all I in 0 .. Consumed - 1 =>
                   After (After'First + I) = Before (Before'First + I)),
     Post               =>
       Spec_Matches (After, Consumed, Data, Position, Index),
     Subprogram_Variant =>
   ...
   begin
      ...
      if Index = Data'Length then
         ...
         if S.Kind = Literal then
            ...
            pragma Assert (S.Length <= Data'Length - Index);
            Lemma_Spec_Frame
              (Before, After, Consumed, Data, S.Position, Index + S.Length);
         end if;
      end if;
   end Lemma_Spec_Frame;

   procedure Lemma_Encoding_Frame
     (Before, After : Byte_Array; Consumed : Natural; Data : Byte_Array) is
   begin
      pragma Assert (Spec_Matches (Before, Consumed, Data, 3, 0));
      Lemma_Spec_Frame (Before, After, Consumed, Data, 3, 0);
      pragma Assert (Spec_Matches (After, Consumed, Data, 3, 0));
   ...
   begin
      ...
      pragma
        Assert
          (Data_Bits (Data, Next)
             = Data_Bits (Data, Position) + Token_Bit_Cost (Data, Position));
      if Next < Count then
   ...
   with
     ...
     Pre                =>
       ...
       and then Spec_Matches (Input, Consumed, Left, Position, Index)
       and then Spec_Matches (Input, Consumed, Right, Position, Index)
       and then (for all I in 0 .. Index - 1 =>
   ...
   procedure Lemma_Prefix_Extend_Match
     (Left, Right : Byte_Array; Index, Length : Natural) is
   begin
      for I in 0 .. Index + Length - 1 loop
         ...
         if I < Index then
            pragma Assert (Left (Left'First + I) = Right (Right'First + I));
         else
         ...
         pragma
           Assert
             (for all I in 0 .. Index + S.Length - 1 =>
                Left (Left'First + I) = Right (Right'First + I));
         Lemma_Spec_Functional
           (Input, Consumed, Left, Right, S.Position, Index + S.Length);
      end if;
   end Lemma_Spec_Functional;

   procedure Lemma_Encoding_Functional
     (Input : Byte_Array; Consumed : Natural; Left, Right : Byte_Array) is
   begin
      pragma Assert (Spec_Matches (Input, Consumed, Left, 3, 0));
      pragma Assert (Spec_Matches (Input, Consumed, Right, 3, 0));
      Lemma_Spec_Functional (Input, Consumed, Left, Right, 3, 0);
   end Lemma_Encoding_Functional;

   procedure Lemma_Shared_Data_Bits (Data : Byte_Array; Count : Natural)
   with
     Ghost,
     Pre                =>
       Data'Length <= Max_Input and then Count <= Data'Length,
     Post               =>
       Inflate.Payload.Data_Bits
   ...
   procedure Lemma_Shared_Data_Bits (Data : Byte_Array; Count : Natural) is
   begin
      if Count > 0 then
   ...
   procedure Lemma_Shared_Encoding (Output : Byte_Array; Data : Byte_Array)
   with
     Ghost,
     Pre  =>
       Output'Length <= Max_Stream_Bytes
       ...
       and then Inflate.Payload.Is_Encoding
                  (Output,
                   ...
                   Data),
     Post =>
       Encodes_Prefix (Output, Data, Data'Length)
       and then Prefix_Value (Output, 3 + Data_Bits (Data, Data'Length), 7)
                = 0;

   procedure Lemma_Shared_Encoding (Output : Byte_Array; Data : Byte_Array) is
   begin
           ...
           Loop_Invariant
             (for all J in 0 .. I - 1 =>
                (if Token_Boundary (Data, J)
                 ...
                 then
                   3 + Data_Bits (Data, J) + 12 <= 8 * Output'Length
                   and then Prefix_Value (Output, 3 + Data_Bits (Data, J), 7)
                  ...
                  pragma
                    Assert
                      (Inflate.Codebooks.Code_Of
                         ...
                         = Token.Distance - 1);
                  pragma
                    Assert (3 + Data_Bits (Data, I) + 12 <= 8 * Output'Length);
                  pragma
                    Assert
                      (Prefix_Value (Output, 3 + Data_Bits (Data, I), 7)
                         = Token.Length - 2);
                  pragma
                    Assert
                      (Prefix_Value (Output, 3 + Data_Bits (Data, I) + 7, 5)
                         = Token.Distance - 1);
               else
                  pragma Assert (Token.Value = Data (Data'First + I));
                  pragma
                    Assert
                      (Inflate.Codebooks.Length_Of
                         (Inflate.Codebooks.Fixed_Literal_Length_Book,
      ...
      pragma Assert (3 + Data_Bits (Data, Data'Length) <= 8 * Output'Length);
      pragma Assert (Encodes_Prefix (Output, Data, Data'Length));
      pragma
        Assert
          (Inflate.Codebooks.Length_Of
             (Inflate.Codebooks.Fixed_Literal_Length_Book, 256)
             = 7);
      pragma
        Assert
          (Inflate.Codebooks.Code_Of
   ...
   begin
      ...
      Inflate.Payload.Serialize
        (Input,
         ...
         3,
         Bits);
      pragma Assert (Bits = 3 + Data_Bits (Input, Input'Length) + 7);
      Lemma_Shared_Encoding (Output, Input);
      Produced := Size;
      pragma Assert (Fixed_Header (Output));
      pragma
        Assert
          (Prefix_Value (Output, 3 + Data_Bits (Input, Input'Length), 7) = 0);
      pragma
        Assert (Produced = (3 + Data_Bits (Input, Input'Length) + 7 + 7) / 8);
     ...
     Contract_Cases     =>
       (Position = End_Position or else Index = End_Index     =>
        ...
        and then Next_Symbol (Input, Position).Kind = Match   =>
          Prefix_Matches'Result
          = (Next_Symbol (Input, Position).Position <= End_Position
             and then Next_Symbol (Input, Position).Length <= End_Index - Index
             and then Match_Applies
                        (Data,
                         Index,
      ...
      case S.Kind is
         ...
         when Match                            =>
            return
              ...
              and then Match_Applies (Data, Index, S.Length, S.Distance)
              and then Prefix_Matches
                         (Input,
                          Data,
                          S.Position,
     ...
     Contract_Cases =>
       (Next_Symbol (Input, Position).Kind = Literal =>
        ...
        Next_Symbol (Input, Position).Kind = Match   =>
          One_Token_Matches'Result
          = (End_Position = Next_Symbol (Input, Position).Position
             and then End_Index = Index + Next_Symbol (Input, Position).Length
             and then Match_Applies
                        (Data,
                         Index,
                         Next_Symbol (Input, Position).Length,
   ...
   with
     ...
     Pre  =>
       ...
       and then End_Position <= 8 * Input'Length
       and then Next_Symbol (Input, Position).Kind = Match
       and then End_Position = Next_Symbol (Input, Position).Position
       and then End_Index = Index + Next_Symbol (Input, Position).Length
       and then Match_Applies
                  (Data,
                   Index,
   ...
   with
     ...
     Pre  =>
       ...
       and then (for all I in 0 .. Count - 1 =>
                   After (After'First + I) = Before (Before'First + I)),
     Post => After (After'First + Position) = Before (Before'First + Position);
   ...
   begin
      for K in 0 .. Length - 1 loop
         ...
         Lemma_Array_Prefix_Element (Before, After, Count, Index + K);
         pragma
           Assert
   ...
   with
     ...
     Pre                =>
       ...
       and then (for all I in 0 .. End_Index - 1 =>
                   After (After'First + I) = Before (Before'First + I)),
     Post               =>
       Prefix_Matches (Input, After, Position, Index, End_Position, End_Index),
     Subprogram_Variant => (Decreases => End_Position - Position);
     ...
     Pre                =>
       ...
       and then One_Token_Matches
                  (Input,
                   ...
                   New_Index),
     Post               =>
       Prefix_Matches (Input, Data, Position, Index, New_Position, New_Index),
   ...
   with
     ...
     Pre  =>
       ...
       and then (for all I in 0 .. Old_Index - 1 =>
                   After (After'First + I) = Before (Before'First + I)),
     Post =>
       Prefix_Matches (Input, After, Position, Index, New_Position, New_Index);
   ...
   begin
      Lemma_Prefix_Data_Frame
        (Input, Before, After, Position, Index, Old_Position, Old_Index);
      Lemma_Prefix_Snoc
        (Input,
         After,
         Position,
   ...
   with
     ...
     Post =>
       ...
       and then Match_Applies (Data, Produced'Old, Length, Distance)
   is
   ...
   begin
      ...
      begin
         loop
            ...
            pragma Loop_Invariant (Spec_Walk (Input, Position, Count).Valid);
            pragma
              Loop_Invariant
            ...
            case S.Kind is
               when Literal           =>
                  ...
                  pragma
                    Assert (Spec_Walk (Input, S.Position, Count + 1).Valid);
                  pragma
                    Assert
               ...
               when Match             =>
                  ...
                  pragma
                    Assert
                      (Spec_Walk (Input, S.Position, Count + S.Length).Valid);
                  pragma
                    Assert
                      (Spec_Walk (Input, S.Position, Count + S.Length).End_Bit
                         = Info.End_Bit);
                  pragma
                    Assert
                      (Count
                         + S.Length
                         + Spec_Walk (Input, S.Position, Count + S.Length)
                             .Decoded_Length
                         = Info.Decoded_Length);
                  declare
                     Before       : constant Byte_Array := Data
                     ...
                     Old_Count    : constant Natural := Count;
                  begin
                     Copy_Selected_Match (Data, Count, S.Length, S.Distance);
                     Position := S.Position;
                     pragma
                       Assert
                         (for all I in 0 .. Old_Count - 1 =>
                     ...
                     pragma Assert (Next_Symbol (Input, Old_Position) = S);
                     pragma
                       Assert
                         (Position
                     ...
                     Lemma_One_Match_Token
                       (Input, Data, Old_Position, Old_Count, Position, Count);
                     pragma
                       Assert
                         (One_Token_Matches
                            (Input,
