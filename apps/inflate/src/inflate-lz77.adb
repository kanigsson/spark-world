...
is
   ...
   begin
      if Distance = 1 then
         ...
         pragma
           Assert (Model.Copies_Match (Before, Output, P, Length, Distance));
         ...
         Output (First + P .. First - 1 + P + Length) :=
           Output (First + P - Distance .. First - 1 + P + Length - Distance);
         Produced := P + Length;
         ...
         pragma
           Assert
             (for all K in 0 .. Length - 1 =>
                Output (First + P + K) = Before (First + P - Distance + K));
         pragma
           Assert (Model.Copies_Match (Before, Output, P, Length, Distance));
         ...
         for K in 0 .. Length - 1 loop
            ...
            Output (First + P + K) := Output (First + P + K - Distance);
