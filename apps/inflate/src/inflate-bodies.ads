...
is
   ...
   is (Consumed in 1 .. Input'Length
       and then ((Input'Length >= 5
                 ...
                 or else (Input'Length <= Fixed.Max_Stream_Bytes
                          and then Data'Length <= Fixed.Max_Input
                          and then Fixed.Is_Encoding (Input, Consumed, Data))
                 or else Dynamic.Decodes (Input, Consumed, Data)))
   with Ghost;
   ...
   function Recognized (Input : Byte_Array; Out_Len : Natural) return Boolean
   is ((Input'Length >= 5
   ...
   function Encoded_Size (Input : Byte_Array) return Natural
   is (if Input'Length >= 5
         and then Model.Stored_Stream_End (Input, Input'First, Input'Last) > 0
       then
   ...
   function Decoded_Size (Input : Byte_Array) return Natural
   is (if Input'Length >= 5
         and then Model.Stored_Stream_End (Input, Input'First, Input'Last) > 0
   ...
   with
     ...
     Pre    =>
       ...
       and then (if Before'Length <= Fixed.Max_Stream_Bytes
                   and then Data'Length <= Fixed.Max_Input
                   and then Fixed.Is_Encoding (Before, Consumed, Data)
                 then After'Length <= Fixed.Max_Stream_Bytes)
       and then (if Dynamic.Decodes (Before, Consumed, Data)
                 then After'Length <= Fixed.Max_Stream_Bytes)
   ...
   with
     ...
     Global => null,
     Pre    =>
       Body_Encodes (Input, Consumed, Left)
