...
is
   ...
   procedure Lemma_First_Code_Separated
     (Counts : Length_Count_Array; Short, Long : Fast_Length) is
   begin
   ...
   procedure Lemma_Prefix_Shorten
     (Bits : Fast_Index; Short, Long : Fast_Length) is
   begin
      if Long > Short + 1 then
         ...
         begin
            ...
            pragma
              Assert
                (P / Q
                   = Stream_Prefix (Bits, Long - 1) / Pow2 (Long - Short - 1));
            pragma
              Assert
   ...
   procedure Lemma_No_Shorter_Code
     (Counts : Length_Count_Array; Bits : Fast_Index; L : Fast_Length) is
   begin
      for M in Fast_Length range 1 .. L - 1 loop
         ...
         pragma
           Assert
             (Stream_Prefix (Bits, M) >= First_Code (Counts, M) + Counts (M));
             ...
             (E
                = Symbols
                    (Count_Before (Counts, From)
                     + (Stream_Prefix (Bits, From)
                        - First_Code (Counts, From)))
                  * 16
                  + From);
      end if;
   ...
   procedure Lemma_Bit_Reverse_Stream (Bits : Fast_Index; L : Fast_Length) is
   begin
      if L > 1 then
         ...
         begin
            ...
            pragma
              Assert
                (Bits mod Pow2 (L) = B * Pow2 (L - 1) + Bits mod Pow2 (L - 1));
         end;
      end if;
   ...
   function Entries_Valid (Table : Huffman_Table) return Boolean
   is (for all B in Fast_Index =>
         Fast_Entry_Valid (Table.Counts, Table.Symbols, B, Table.Map (B)))
   with Ghost, Pre => Valid_Counts (Table.Counts);

   function Map_Grows (Before, After : Fast_Map) return Boolean
   is (for all B in Fast_Index => Before (B) = 0 or else After (B) /= 0)
   with Ghost;

   function Covered_Through
     (Counts : Length_Count_Array; Map : Fast_Map; Last : Natural)
      return Boolean
   is (for all B in Fast_Index =>
         Map (B) /= 0
         or else (for all L in Fast_Length =>
   ...
   function Length_Covered
     (Counts : Length_Count_Array; Map : Fast_Map; L : Fast_Length)
      return Boolean
   is (for all B in Fast_Index =>
   ...
   with
     Ghost,
     Pre  => R < Step,
     Post => (for all B in Fast_Index => B >= R or else B mod Step /= R);
   ...
   with
     ...
     Post =>
       (for all B in Fast_Index =>
          B >= J + Step or else B <= J or else B mod Step /= J mod Step);

   procedure Lemma_Remainder_Window (J : Fast_Index; Step : Fast_Step) is null;

   procedure Lemma_Covered_Through_Grows
     (Counts : Length_Count_Array; Before, After : Fast_Map; Last : Natural)
   with
     Ghost,
     Pre  =>
       Covered_Through (Counts, Before, Last)
   ...
   with
     ...
     Pre  =>
       Next_Code < Pow2 (L)
       and then Covered_Before (Before, L, First_Code, Next_Code)
       and then Map_Grows (Before, After)
       and then (for all B in Fast_Index =>
                   Stream_Prefix (B, L) /= Next_Code or else After (B) /= 0),
     Post => Covered_Before (After, L, First_Code, Next_Code + 1);
   ...
   with
     ...
     Pre  =>
       Covered_Before
         (Map, L, First_Code (Counts, L), First_Code (Counts, L) + Counts (L)),
   ...
   with
     ...
     Pre  =>
       ...
       and then After (J) /= 0
       and then (for all B in Fast_Index =>
                   B >= J or else B mod Step /= R or else Before (B) /= 0),
     Post =>
       (for all B in Fast_Index =>
          B >= J + Step or else B mod Step /= R or else After (B) /= 0);
   ...
   begin
      for B in Fast_Index loop
         ...
         if Stream_Prefix (B, L) = Code then
            pragma Assert (B mod Step = Bit_Reverse (Code, L));
   ...
   with
     Pre  =>
       Valid_Counts (Table.Counts)
       and then Entries_Valid (Table)
       and then Covered_Through (Table.Counts, Table.Map, Done)
       and then Code >= First_Code (Table.Counts, L)
       and then Code - First_Code (Table.Counts, L) < Table.Counts (L)
     ...
     Post =>
       ...
       and then Covered_Through (Table.Counts, Table.Map, Done)
       and then (for all B in Fast_Index =>
                   Stream_Prefix (B, L) /= Code or else Table.Map (B) /= 0)
   is
      Step     : constant Fast_Step := Pow2 (L);
      E        : constant Packed_Entry :=
        Table.Symbols (Symbol_Index) * 16 + L;
      Before   : constant Fast_Map := Table.Map
      with Ghost;
      Previous : Fast_Map
      with Ghost;
      J        : Natural;
   begin
      ...
      while J <= Fast_Index'Last loop
         pragma Loop_Invariant (J mod Step = Bit_Reverse (Code, L));
         ...
         Lemma_Extend_Remainder
           (Previous, Table.Map, Fast_Index (J), Step, Bit_Reverse (Code, L));
         Lemma_Mod_Add_Step (Fast_Index (J), L);
         J := J + Step;
      end loop;
      Lemma_Filled_Code (Table.Map, L, Code, Step, J);
      Lemma_Covered_Through_Grows (Table.Counts, Before, Table.Map, Done);
   end Fill_Code;
   ...
   with
     ...
     Pre  =>
       Valid_Counts (Table.Counts)
       and then Entries_Valid (Table)
       and then Covered_Through (Table.Counts, Table.Map, Fast_Bits),
     Post =>
       (for all B in Fast_Index =>
          Table.Map (B) = Reference_Entry (Table.Counts, Table.Symbols, B))
   is
   begin
      for B in Fast_Index loop
         pragma
   ...
   procedure Build_Fast (Table : in out Huffman_Table) is
      ...
      Before           : Fast_Map
      with Ghost;
      Original_Counts  : constant Length_Count_Array := Table.Counts
      with Ghost;
      Original_Symbols : constant Symbol_Map := Table.Symbols
      with Ghost;
   begin
      Table.Map := (others => 0);
      for Len in Fast_Length loop
         ...
         pragma Loop_Invariant (Entries_Valid (Table));
         pragma
           Loop_Invariant (Covered_Through (Table.Counts, Table.Map, Done));
         ...
         for K in 0 .. Count - 1 loop
            ...
            pragma
              Loop_Invariant
                (Covered_Before (Table.Map, Len, Code, Code + Built));
            Before := Table.Map;
            pragma Assert (Covered_Through (Table.Counts, Before, Done));
            pragma Assert (Covered_Before (Before, Len, Code, Code + Built));
            Fill_Code (Table, Len, Code + K, Index + K, Done);
            pragma Assert (Map_Grows (Before, Table.Map));
            Lemma_Extend_Coverage (Before, Table.Map, Len, Code, Code + Built);
            Built := Built + 1;
         end loop;
      ...
      pragma Assert (Covered_Through (Table.Counts, Table.Map, Fast_Bits));
