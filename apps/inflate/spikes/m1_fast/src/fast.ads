package Fast with SPARK_Mode => On is
   ...
   is
     (if L = 1 then 0 else Count_Before (Counts, L - 1) + Counts (L - 1))
   with
   ...
   is
     (if L = 1 then Bits mod 2
      else 2 * Stream_Prefix (Bits, L - 1) + (Bits / Pow2 (L - 1)) mod 2)
   with
     Subprogram_Variant => (Decreases => L),
     Post => Stream_Prefix'Result < Pow2 (L);

   function Valid_Counts (Counts : Length_Count_Array) return Boolean is
   ...
   with
     ...
     Always_Terminates,
     Pre  => Short < Long,
     Post => First_Code (Counts, Long) / Pow2 (Long - Short) >=
   ...
   function Reference_Entry
     (Counts  : Length_Count_Array;
      ...
      return Packed_Entry
   is
     (if Stream_Prefix (Bits, L) >= First_Code (Counts, L)
          and then Stream_Prefix (Bits, L) - First_Code (Counts, L) < Counts (L)
      then Symbols
             (Count_Before (Counts, L)
              + (Stream_Prefix (Bits, L) - First_Code (Counts, L)))
             * 16 + L
      elsif L = Fast_Bits then 0
   ...
   with
     ...
     Post => (for all M in Fast_Length range 1 .. L - 1 =>
   ...
   function Bit_Reverse (V : Natural; L : Fast_Length) return Natural is
     (if L = 1 then V mod 2
      elsif Bit_Reverse (V / 2, L - 1) >= Pow2 (L - 1) then 0
   ...
   with
     Ghost,
     Always_Terminates,
     Post => Bit_Reverse (Stream_Prefix (Bits, L), L) =
               Bits mod Pow2 (L),
     Subprogram_Variant => (Decreases => L);
   ...
   with
     Ghost,
     Always_Terminates,
     Post => Stream_Prefix (Bits, L) =
               Stream_Prefix (Bits mod Pow2 (L), L),
     Subprogram_Variant => (Decreases => L);
   ...
   with
     Ghost,
     Pre  => L > 1,
     Post => (Bits mod Pow2 (L)) mod Pow2 (L - 1) =
               Bits mod Pow2 (L - 1)
   ...
   with
     Ghost,
     Post => Bits mod Pow2 (L) =
               ((Bits / Pow2 (L - 1)) mod 2) * Pow2 (L - 1)
               + Bits mod Pow2 (L - 1);

   procedure Lemma_Mod_Add_Step (X : Fast_Index; L : Fast_Length)
   with
     Ghost,
     Post => (X + Pow2 (L)) mod Pow2 (L) = X mod Pow2 (L);
