...
package Huffman_Round_Trip with SPARK_Mode => On is
   ...
   function First_Code
     (Counts : Kraft.Length_Count_Array;
      L      : Kraft.Code_Length_Pos) return Natural
   is
     (if L = 1 then 0
      else 2 * (First_Code (Counts, L - 1) + Counts (L - 1)))
   with
     Subprogram_Variant => (Decreases => L),
     Post => First_Code'Result <= Kraft.Max_Symbols * (Kraft.Pow2 (L) - 2);

   function Count_Length
     (Lengths : Full_Length_Array;
      L       : Kraft.Code_Length_Pos;
      Hi      : Natural) return Kraft.Symbol_Count
   is
     (if Hi = 0 then 0
      else Count_Length (Lengths, L, Hi - 1)
   ...
   function Kraft_Value
     (Counts : Kraft.Length_Count_Array;
      L      : Kraft.Code_Length) return Natural
   is
     (if L = 0 then 0
      else 2 * Kraft_Value (Counts, L - 1) + Counts (L))
   with
     Subprogram_Variant => (Decreases => L),
     Post => Kraft_Value'Result <=
               Kraft.Max_Symbols * (Kraft.Pow2 (L) - 1);
   ...
   function Complete (Book : Codebook) return Boolean is
     (Kraft_Value (Book.Counts, 15) = Kraft.Pow2 (15));

   function Ready (Book : Codebook) return Boolean is
     (Exact_Counts (Book) and then Canonical_Valid (Book) and then Complete (Book));
   ...
   is
     (First_Code (Book.Counts, Book.Lengths (Symbol)) + Rank_Of (Book, Symbol))
   with
     Pre  => Book.Lengths (Symbol) /= 0
   ...
   is
     (if Length = 0 then 0
      else 2 * Prefix_Value (Bits, Start, Length - 1)
           + Bits (Start + Length - 1))
   with
   ...
   is
     (if Count = 0 then 0
      else Encoded_Length (Book, Input, Count - 1)
           + Book.Lengths (Input (Count - 1)))
   with
     Pre => (for all I in Message_Index range 0 .. Count - 1 =>
   ...
   is
     (Length = Encoded_Length (Book, Symbols, Symbol_Count)
      and then
        (for all I in Message_Index range 0 .. Symbol_Count - 1 =>
           Prefix_Value
             (Bits,
              Encoded_Length (Book, Symbols, I),
              Book.Lengths (Symbols (I))) = Code_Of (Book, Symbols (I))))
   with
     Pre => Ready (Book)
              and then
            (for all I in Message_Index range 0 .. Symbol_Count - 1 =>
   ...
   with
     Global => null,
     Post   => Book.Lengths = Lengths
               and then (if Success then Ready (Book));
   ...
   with
     ...
     Pre  => Ready (Book)
               and then
             (for all I in Message_Index range 0 .. Input_Count - 1 =>
                Book.Lengths (Input (I)) /= 0),
     Post => Length = Encoded_Length (Book, Input, Input_Count)
