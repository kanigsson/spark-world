package body Huffman_Round_Trip with SPARK_Mode => On is
   ...
   with
     ...
     Post => Count_Length (Lengths, L, Lo) <= Count_Length (Lengths, L, Hi),
   ...
   with
     Ghost,
     Pre  => Pos < Hi and then Hi <= Lengths'Last + 1
               and then Lengths (Pos) = L,
     Post => Count_Length (Lengths, L, Hi) >
               Count_Length (Lengths, L, Pos),
   ...
   with
     Ghost,
     Pre  => N > 1,
     Post => Kraft.Pow2 (N) = 2 * Kraft.Pow2 (N - 1);
   ...
   procedure Lemma_Div_Substitute
     (A, A2 : Natural; D, D2 : Positive)
   with
     Ghost,
     Pre  => A = A2 and then D = D2,
     Post => A / D = A2 / D2;
   ...
   with
     Ghost,
     Post => Kraft_Value (Counts, L) = Kraft.Kraft_Sum (Counts, L),
   ...
   with
     Ghost,
     Pre  => L >= 1,
     Post => First_Code (Counts, L) = 2 * Kraft_Value (Counts, L - 1),
     Subprogram_Variant => (Decreases => L);
   ...
   with
     ...
     Pre  => Short < Long
               and then
             (for all L in Kraft.Code_Length_Pos =>
                First_Code (Counts, L) + Counts (L) <= Kraft.Pow2 (L)),
     Post => First_Code (Counts, Long) / Kraft.Pow2 (Long - Short) >=
   ...
   begin
      if Long > Short + 1 then
         ...
         pragma Assert (Kraft.Pow2 (Long - Short) =
                          2 * Kraft.Pow2 (Long - Short - 1));
         declare
   ...
   with
     Ghost,
     Pre  => Short < Long and then Start + Long <= Max_Message_Bits,
     Post => Prefix_Value (Bits, Start, Short) =
      ...
      if Long > Short + 1 then
         ...
         begin
            ...
            Lemma_Div_Substitute
              (Kraft.Pow2 (Long - Short), 2 * Q, 1, 1);
            Lemma_Div_Substitute
              (Prefix_Value (Bits, Start, Long), 2 * P + B,
               Kraft.Pow2 (Long - Short), 2 * Q);
            pragma Assert
              (Prefix_Value (Bits, Start, Long) /
                 Kraft.Pow2 (Long - Short) =
   ...
   with
     ...
     Pre  => Ready (Book)
               ...
               and then Prefix_Value
                          (Bits, Start, Book.Lengths (Expected)) =
                            Code_Of (Book, Expected),
     Post => Prefix_Value (Bits, Start, Short) <
               First_Code (Book.Counts, Short)
   ...
   begin
      ...
      pragma Assert (Code_Of (Book, Expected) >= First_Code (Book.Counts, Long));
      Lemma_Div_Monotone
        (Code_Of (Book, Expected), First_Code (Book.Counts, Long),
         Kraft.Pow2 (Long - Short));
      Lemma_Div_Substitute
        (Prefix_Value (Bits, Start, Long), Code_Of (Book, Expected),
         Kraft.Pow2 (Long - Short), Kraft.Pow2 (Long - Short));
      pragma Assert
        (Prefix_Value (Bits, Start, Short) >=
   ...
   with
     ...
     Pre  => Ready (Book)
               ...
               and then Prefix_Value
                          (Bits, Start, Book.Lengths (Expected)) =
                            Code_Of (Book, Expected),
     Post => (if L < Book.Lengths (Expected)
              then Prefix_Value (Bits, Start, L) <
                     First_Code (Book.Counts, L)
   ...
   procedure Lemma_Rank_Unique
     (Book : Codebook;
      A, B : Kraft.Symbol_Value)
   with
     Ghost,
     Pre  => Exact_Counts (Book)
   ...
   with
     ...
     Pre  => Start + Length <= Max_Message_Bits
               and then
             (for all K in Bit_Index range Start .. Start + Length - 1 =>
                After (K) = Before (K)),
   ...
   with
     ...
     Post => Prefix_Value (Bits, Start, L) =
               2 * Prefix_Value (Bits, Start, L - 1)
                 + Bits (Start + L - 1);
   ...
   with
     Pre  => Start + Length <= Max_Message_Bits
               and then Code < Kraft.Pow2 (Length),
     Post => Prefix_Value (Bits, Start, Length) = Code
               and then
             (for all K in Bit_Index =>
   ...
   with
     ...
     Pre  => Lo < Hi
               and then
             (for all I in Message_Index range 0 .. Hi - 1 =>
                Book.Lengths (Input (I)) /= 0),
     Post => Encoded_Length (Book, Input, Lo) <
   ...
   with
     ...
     Pre  => Pos < Hi
             ...
             (for all I in Message_Index range 0 .. Hi - 1 =>
                Book.Lengths (Input (I)) /= 0),
     Post => Encoded_Length (Book, Input, Pos)
               + Book.Lengths (Input (Pos)) <=
             Encoded_Length (Book, Input, Hi);
   ...
   with
     ...
     Post => Book.Lengths (Symbol) = Length
               and then Rank_Of (Book, Symbol) = Rank;
   ...
   begin
      ...
      for S in Kraft.Symbol_Value loop
         pragma Loop_Invariant
           (Seen = Count_Length (Book.Lengths, Length, S));
   ...
   begin
      ...
      Kraft.Construct
        (Lengths, Book.Counts, Symbols, Is_Complete, Is_Valid);
      ...
      for I in Message_Index range 0 .. Input_Count - 1 loop
         ...
         pragma Loop_Invariant
           (for all J in Message_Index range 0 .. I - 1 =>
              Prefix_Value
                (Bits, Encoded_Length (Book, Input, J),
                 Book.Lengths (Input (J))) = Code_Of (Book, Input (J)));
         declare
            L : constant Kraft.Code_Length_Pos := Book.Lengths (Input (I));
            C : constant Natural := Code_Of (Book, Input (I));
            Before : constant Bit_Buffer := Bits with Ghost;
         begin
            ...
            for J in Message_Index range 0 .. I - 1 loop
               pragma Loop_Invariant
                 (for all K in Message_Index range 0 .. J - 1 =>
                    Prefix_Value
                      (Bits, Encoded_Length (Book, Input, K),
                       Book.Lengths (Input (K))) = Code_Of (Book, Input (K)));
               Lemma_Segment_Before (Book, Input, J, I);
               pragma Assert
                 (for all K in Bit_Index range
                    Encoded_Length (Book, Input, J) ..
                    Encoded_Length (Book, Input, J)
                      + Book.Lengths (Input (J)) - 1 =>
                    Bits (K) = Before (K));
               Lemma_Prefix_Frame
                 (Before, Bits, Encoded_Length (Book, Input, J),
      ...
      while Cursor < Length loop
         ...
         declare
            Expected : constant Kraft.Symbol_Value :=
              Input (Restored_Count) with Ghost;
            Expected_Length : constant Kraft.Code_Length_Pos :=
              Book.Lengths (Expected) with Ghost;
            Code  : Natural := 0;
            First : Natural := 0;
            Found : Boolean := False;
         begin
            ...
            for L in Kraft.Code_Length_Pos loop
               ...
               pragma Loop_Invariant
                 (Code = Prefix_Value (Bits, Cursor, L - 1));
               ...
               Lemma_Shorter_Or_Equal
                 (Book, Bits, Cursor, Expected, L);
