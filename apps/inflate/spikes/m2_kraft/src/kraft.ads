...
is
   ...
   --  Sum of Counts (1 .. L)
   function Sum_Counts (C : Length_Count_Array; L : Code_Length) return Natural
   is (if L = 0 then 0 else Sum_Counts (C, L - 1) + C (L))
   with
     Ghost,
   ...
   function Count_Len
     (Lengths : Code_Length_Array; L : Code_Length_Pos; Hi : Natural)
      return Natural
   is (if Hi <= Lengths'First
       then 0
       else
         Count_Len (Lengths, L, Hi - 1)
   ...
   function Kraft_Sum (C : Length_Count_Array; L : Code_Length) return Natural
   is (if L = 0 then 0 else 2 * Kraft_Sum (C, L - 1) + C (L))
   with
     Ghost,
     Subprogram_Variant => (Decreases => L),
   ...
   with
     ...
     Post               =>
       (if L <= Up_To
        then
          Sum_Counts ((C with delta L => V), Up_To)
          = Sum_Counts (C, Up_To) + V - C (L)
        else
          Sum_Counts ((C with delta L => V), Up_To) = Sum_Counts (C, Up_To));

   --  Prefix sums of a nonnegative sequence are monotone.
   procedure Lemma_Sum_Monotone
     (C : Length_Count_Array; L1 : Code_Length; L2 : Code_Length)
   with
     Ghost,
     Pre                => L1 <= L2,
   ...
   with
     Global => null,
     Pre    =>
       Bits'Length >= 15 and then Sum_Counts (Counts, 15) <= Max_Symbols;
