...
is
   ...
   procedure Lemma_Sum_Monotone
     (C : Length_Count_Array; L1 : Code_Length; L2 : Code_Length) is
   begin
      if L1 < L2 then
   ...
   begin
      ...
      for I in Lengths'Range loop
         ...
         pragma Loop_Invariant (Sum_Counts (Counts, 15) <= I - Lengths'First);
         if Lengths (I) /= 0 then
            Lemma_Sum_Update
      ...
      for I in Lengths'Range loop
         pragma
           Loop_Invariant
             (for all L in Code_Length_Pos =>
                Offs (L)
                = Sum_Counts (Counts, L - 1) + Count_Len (Lengths, L, I));
         if Lengths (I) /= 0 then
            Lemma_Count_Len_Step (Lengths, Lengths (I), I, Lengths'Last + 1);
