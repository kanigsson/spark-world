package BWT.Ranks with SPARK_Mode is
   type Mapping is array (Positive range <>) of Positive;

   function Ordered (Last : String; A, B : Positive) return Boolean is
     (Last (A) < Last (B) or else (Last (A) = Last (B) and then A <= B))
   with Pre => Supported (Last) and then A in Last'Range and then B in Last'Range;
   ...
   with Ghost,
        Pre => Supported (Last) and then A in Last'Range and then B in Last'Range
          and then A /= B and then Ordered (Last, A, B),
        Post => Rank (Last, A, Last'Length) < Rank (Last, B, Last'Length);

   function Permutation (Map : Mapping) return Boolean is
     (Map'First = 1 and then Map'Length <= Max_Length
      and then (for all P of Map => P in Map'Range)
      and then (for all I in Map'Range =>
        (for all J in Map'Range => (if I /= J then Map (I) /= Map (J)))))
   with Ghost;

   function LF (Last : String) return Mapping
   with Pre => Supported (Last),
        Post => LF'Result'First = 1 and then LF'Result'Length = Last'Length
          and then Permutation (LF'Result)
          and then (for all I in Last'Range =>
            LF'Result (I) = Rank (Last, I, Last'Length))
          and then (for all I in Last'Range => (for all J in Last'Range =>
            (Ordered (Last, I, J) = (LF'Result (I) <= LF'Result (J)))));
   ...
   with Ghost,
        Pre => Supported (Last) and then Primary in Last'Range
          and then Steps < Last'Length,
        Post => Walk (Last, Primary, 0) = Primary
