...
package body Git_Changes.Repositories is
   ...
   begin
      Item := (others => <>);
      Error := (others => <>);
      Query (Path, "--is-bare-repository", "discover repository", Bare_Output, Local_Error);
      if not Success (Local_Error) then
         Set_Error
           (Error, Invalid_Repository, "discover repository", Detail (Local_Error),
            Exit_Status (Local_Error));
         return;
      end if;
      Bare := Git_Changes.Backends.Trim_Line_End (To_String (Bare_Output)) = "true";

      Query (Path, "--absolute-git-dir", "resolve git directory", Git_Output, Error);
      if not Success (Error) then
         return;
      end if;
      Query (Path, "--show-object-format", "resolve object format", Format_Output, Error);
      if not Success (Error) then
         return;
      end if;
      ...
      if Bare then
         ...
         exception
            when others =>
               Set_Error (Error, Invalid_Repository, "discover repository", "invalid path");
               return;
         end;
      else
      ...
      Item :=
        (Opened => True,
         ...
         Git_Dir => To_Unbounded_String
           (Git_Changes.Backends.Trim_Line_End (To_String (Git_Output))),
         Object_Format_Name => To_Unbounded_String
