...
package body Box_Demo is
   ...
   function Glyph (Ch : Character) return Wide_Wide_Character is
   ...
   begin
      ...
      Put_Text (S, 1, 1,
                ...
                FG => Title_FG, Attr => (Bold => True, others => False));

      --  Need a few rows for title + box + status to fit; otherwise just show
      --  the title and status and skip the box.
      if S.Rows >= Row_Count (Box_H + 3) and then S.Cols >= Col_Count (Box_W + 2)
      then
            ...
            for DC in 0 .. Box_W - 1 loop
               declare
                  ...
                  Border : constant Boolean :=
                    DR = 0 or else DR = Box_H - 1
                    or else DC = 0 or else DC = Box_W - 1;
               begin
                  if Border then
                     Set (S, R, C,
                          (Glyph      => Glyph ('#'),
         ...
         Put_Text (S, Row_Index (Box_Top + 2), Col_Index (Box_Left + 2),
                   "Hello, terminal!", BG => Fill_BG);
      end if;

      --  Status row (bottom), shown inverse.
      declare
         Status : constant String :=
           "size" & S.Rows'Image & " x" & S.Cols'Image
           & "   last key: " & To_String (Last_Key);
      begin
         Put_Text (S, S.Rows, 1, Status, Attr => (Inverse => True, others => False));
      end;
   end Paint;
   ...
   begin
      ...
      case Event.Kind is
         when Up    => Box_Top  := Box_Top  - 1;
         when Down  => Box_Top  := Box_Top  + 1;
         when Left  => Box_Left := Box_Left - 1;
         when Right => Box_Left := Box_Left + 1;
         when Escape =>
            Quit := True;
         when Char =>
            ...
            if Event.Code = Character'Pos ('q')
              ...
              or else (Event.Mods.Ctrl and then Event.Code = Character'Pos ('C'))
            then
               Quit := True;
            end if;
         when others =>
