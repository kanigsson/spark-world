...
begin
   ...
   begin
      ...
      Check (First.Col = 2 and then Last.Col = 9,
   ...
   begin
      ...
      Check (P (1).Cols + P (2).Cols + P (3).Cols + 2 = 100,
             "the panes and their separators cover the width exactly");
      Check (P (1).Start_Col = 1
      ...
      Check (P (1).Cols = 30 and then P (2).Cols = 25,
      ...
      Check (P (1).Cols + P (2).Cols + P (3).Cols = 100
      ...
      Check (P (2).Cols = 0 and then P (1).Cols > 0 and then P (3).Cols > 0,
             "the most disposable pane gives up its place first");
      Check (P (1).Cols + P (3).Cols + 1 = 55,
      ...
      Check (P (1).Cols = 0 and then P (2).Cols = 0 and then P (3).Cols = 0,
             "a zero-width terminal paints nothing");

      Lay.Compute (Spec, 100, 2, True, True, Lay.Drop_By_Priority, P);
      Check (P (2).Cols = 100 and then P (2).Start_Col = 1
             and then P (1).Cols = 0 and then P (3).Cols = 0,
   ...
   declare
      P : Lay.Placement_Array (1 .. 2) :=
        (1 => (Start_Col => 1, Cols => 10),
         2 => (Start_Col => 12, Cols => 8));
      H : Lay.Hit;
   begin
      H := Lay.Locate (P, 1, 5, Col => 12, Row => 3);
      Check (H.Kind = Lay.Pane_Hit and then H.Pane = 2
             and then H.Local_Col = 1 and then H.Local_Row = 3,
      ...
      Check (Lay.Locate (P, 3, 5, 1, 2).Kind = Lay.Nowhere,
             "a row reserved for chrome is nowhere");
      Check (Lay.Locate (P, 1, 5, 21, 1).Kind = Lay.Nowhere,
             "past the last pane is nowhere");
      Check (Lay.Locate (P, 1, 5, 0, 1).Kind = Lay.Nowhere,
      ...
      Check (Lay.Locate (P, 1, 5, 11, 1).Kind = Lay.Nowhere,
   ...
   begin
      ...
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Press, 3, 2), P, F, 1, 5, 1,
                 Rules, G);
      Check (G.Kind = Gest.Click and then G.Pane = 1
             and then G.On_Line and then G.At_Line = 2
      ...
      Check (not Gest.Has_Selection (R),
             "a press alone is not a selection");
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Motion, 6, 4), P, F, 1, 5, 1,
                 Rules, G);
      Check (G.Kind = Gest.Range_Extended
             and then G.First = (Line => 2, Col => 2)
             and then G.Last = (Line => 4, Col => 5),
             "a drag extends a range in document coordinates");
      Check (Gest.Has_Selection (R) and then Gest.Selected_Pane (R) = 1,
             "the range is retained for the host to paint");

      --  Motion into another pane is ignored: the drag stays where it began.
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Motion, 14, 4), P, F, 1, 5, 1,
                 Rules, G);
      Check (G.Kind = Gest.None, "a drag does not cross a pane boundary");
      Gest.Selected_Range (R, First, Last);
      Check (Last = (Line => 4, Col => 5),
             "a drag that left its pane keeps the range it had");

      --  Releasing commits it.
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Release, 6, 4), P, F, 1, 5, 1,
                 Rules, G);
      Check (G.Kind = Gest.Range_Committed and then G.Pane = 1,
             "releasing commits the range");

      --  A press and release without motion is a click, not a selection.
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Press, 3, 2), P, F, 1, 5, 1,
                 Rules, G);
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Release, 3, 2), P, F, 1, 5, 1,
                 Rules, G);
      Check (G.Kind = Gest.Range_Cancelled and then not Gest.Has_Selection (R),
             "a click without motion copies nothing");

      --  A press below the end of the document focuses but names no line.
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Press, 3, 5), P, F, 1, 5, 1,
                 Rules, G);
      Check (G.Kind = Gest.Click and then not G.On_Line,
             "a press below the text is still a click on the pane");

      --  Shift hands the drag back to the terminal.
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Press, 3, 2, Shift => True),
                 P, F, 1, 5, 1, Rules, G);
      ...
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Press, 11, 2), P, F, 1, 5, 1,
                 Rules, G);
      Check (G.Kind = Gest.None, "pressing a separator moves nothing yet");
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Motion, 15, 2), P, F, 1, 5, 1,
                 Rules, G);
      Check (G.Kind = Gest.Separator_Dragged and then G.Pane = 1
             and then G.Split_Col = 15,
             "dragging a separator reports the column it moved to");
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Motion, 90, 2), P, F, 1, 5, 1,
                 Rules, G);
      Check (G.Kind = Gest.Separator_Dragged,
             "a separator drag keeps going past the panes");
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Release, 90, 2), P, F, 1, 5, 1,
                 Rules, G);
      Gest.Feed (R, Mouse (Tui.Input.Mouse_Motion, 15, 2), P, F, 1, 5, 1,
                 Rules, G);
   ...
   declare
      ...
      G : Gest.Gesture;
   begin
      Gest.Feed (R, Mouse (Tui.Input.Wheel_Down, 14, 2), P, F, 1, 5,
                 Focused => 1,
                 Rules   => (Wheel => Gest.Pane_Under_Cursor, others => <>),
                 Result  => G);
      Check (G.Kind = Gest.Wheel and then G.Pane = 2
             and then not G.Takes_Focus and then not G.Upward
             and then G.Notches = 3,
             "the wheel scrolls the pane under the cursor without the keyboard");

      Gest.Feed (R, Mouse (Tui.Input.Wheel_Up, 14, 2), P, F, 1, 5,
                 Focused => 1,
                 Rules   => (Wheel => Gest.Focused_Pane, others => <>),
                 Result  => G);
      Check (G.Kind = Gest.Wheel and then G.Pane = 1 and then G.Upward,
             "a keyboard-centric wheel scrolls the focused pane instead");

      Gest.Feed (R, Mouse (Tui.Input.Wheel_Down, 14, 2), P, F, 1, 5,
                 Focused => 1,
                 Rules   => (Wheel  => Gest.Under_Cursor_And_Focus,
                             others => <>),
                 Result  => G);
      Check (G.Kind = Gest.Wheel and then G.Pane = 2 and then G.Takes_Focus,
      ...
      Gest.Feed (R, Mouse (Tui.Input.Wheel_Down, 40, 2), P, F, 1, 5,
                 Focused => 1,
                 Rules   => (Wheel => Gest.Pane_Under_Cursor, others => <>),
                 Result  => G);
   ...
   begin
      ...
      Check (Clip.Length (Text) = 3 and then not Cut,
      ...
      Check (Clip.Length (Text) = 4, "the cell under the last column is copied");
   ...
   begin
      ...
      for Step in 1 .. 9 loop
         PList.Move (E, PList.Sel_Down, Content (1 .. 100), Idx,
                     Sel_Line, Moved);
      end loop;
      Check (Sel_Line = 10 and then Eng.Top_Line (E) = 1,
   ...
   begin
      Mark.Row (S, 2);
      Check (Get (S, 2, 1).Attributes.Inverse
      ...
      Mark.Overlay (S, Top => 1, Left => 0,
                    First => (Line => 2, Col => 0),
                    Last  => (Line => 2, Col => 3));
      Check (not Get (S, 2, 1).Attributes.Inverse,
             "a selection over an inverse row toggles back to normal");
      Mark.Overlay (S, Top => 1, Left => 0,
                    First => (Line => 1, Col => 1),
                    Last  => (Line => 1, Col => 2));
      Check (not Get (S, 1, 1).Attributes.Inverse
      ...
      declare
         T : Surface := Blank (3, 4);
      begin
         Mark.Overlay (T, Top => 1, Left => 0,
                       First => (Line => 1, Col => 2),
                       Last  => (Line => 3, Col => 1));
         Check (not Get (T, 1, 2).Attributes.Inverse
      ...
      declare
         T : Surface := Blank (1, 4);
      begin
         Mark.Overlay (T, Top => 1, Left => 5,
                       First => (Line => 1, Col => 6),
                       Last  => (Line => 1, Col => 6));
         Check (Get (T, 1, 2).Attributes.Inverse
